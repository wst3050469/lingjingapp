...
1. **替换图片上传为文件上传按钮**：InputToolbar中原"上传图片"按钮改造为"上传文件"按钮，图标和tooltip相应更新
   ...
   a. 当用户查看输入工具栏时，系统应展示"上传文件"按钮（使用文件/回形针图标）

2. **文件选择对话框支持多格式**：点击上传文件按钮触发的文件选择对话框必须支持图片和文档文件格式

   a. 当用户点击上传文件按钮时，文件选择对话框的accept过滤器应包含image/*、.pdf、.txt、.md、.doc、.docx、.xls、.xlsx

3. **图片文件处理**：用户上传的图片文件仍按现有逻辑以base64 DataURL方式读取和展示

   a. 当用户上传PNG/JPG等图片文件时，系统应以缩略图chip展示，并以base64编码随消息发送

4. **文档文件处理**：用户上传的PDF/TXT/DOC/DOCX等文档文件必须解析为文本内容

   a. 当用户上传PDF文件时，系统应调用electronAPI.context.parseDocument解析文件内容，解析成功后以附件chip展示
...
1. **图片附件chip样式**：图片附件chip必须展示缩略图预览、文件名和移除按钮
   ...
   a. 当图片附件chip被渲染时，chip应包含缩略图、截断文件名和×移除按钮

2. **文档附件chip样式**：文档附件chip必须展示格式图标、文件名、内容长度摘要和移除按钮

   a. 当文档附件chip被渲染时，chip应包含格式对应图标、文件名、字符数摘要和×移除按钮

3. **chip颜色区分**：不同类型附件chip应使用不同的颜色风格以便视觉区分

   a. 当图片附件chip展示时，系统应使用白色/默认边框；当文档附件chip展示时，系统应使用紫色边框和背景

4. **chip悬停交互**：chip的移除按钮仅在鼠标悬停时显示

   a. 验收条件：While 鼠标未悬停在chip上，the 移除按钮应不可见；当鼠标悬停在chip上时，移除按钮应可见
...
1. **maxFileSize**：单文件最大大小，默认25MB
2. **maxTotalAttachmentSize**：单次发送附件总大小上限，默认50MB
3. **supportedImageFormats**：支持的图片格式，默认[png,jpg,jpeg,gif,webp,svg,bmp]
4. **supportedDocumentFormats**：支持的文档格式，默认[pdf,txt,md,doc,docx,xls,xlsx]
5. **maxParseTimeout**：文档解析超时时间，默认30秒
...
| 问题编号 | 问题描述 | 修复方式 |
|---------|---------|---------|
| G-1 | 验收条件格式不一致 | 统一为当...时，系统应...中文EARS格式 |
| G-2 | EARS中英混杂 | When→当，shall→应，If→若，While→当 |
| G-3 | 章节编号体系不一致 | 统一为纯数字层级编号，去除加粗包裹 |
| G-4 | 验收条件标记符号不一致 | 统一为a./b./c.字母序号 |
| G-5 | 术语定义位置不一致 | 建立共享术语表引用 |
| M-8 | 配置默认值描述不完整 | 补充6.3配置默认值章节 |
