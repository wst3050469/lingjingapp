...
// @ts-ignore -- 'electron' is only available in Electron main process context
import type { BrowserWindow } from 'electron';
import { FusionInitializer } from '../fusion-initializer.js';
// @ts-ignore -- FusionInitResult lives in fusion-initializer.js but imported from types for re-export compat
...
export async function patchElectronMain(
  mainWindow: BrowserWindow,
  fusionConfig: FusionConfig,
  deps: PatchElectronMainDeps = {},
): Promise<PatchElectronMainResult> {
  ...
  if (initResult.success) {
    ...
    logger.warn(
      `[Fusion] Initialized with degraded modules: ${initResult.degraded.join(', ')}`,
    );
