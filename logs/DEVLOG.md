...
## 2026-07-07 (午后续) — 网站内容修正 ✅

### 修复内容
- 更新日志版本号去重: 全部写成v1.73.188 → 修正为v1.73.188/185/180/175/170
- v1.73.188说明更新: "录音权限修复 + expo-av语音插件 + v2v3签名 + OSS直连"
- 清理所有GitHub占位链接 `github.com/your-repo/lingjing` → `#`

---

## 2026-07-07 (午后) — Cloud端口安全加固 ✅

### 操作
- Cloud服务 (8000) 绑定从 `0.0.0.0` 改为 `127.0.0.1`
- 修改文件: `/opt/cloud-server/server.js` + `systemctl restart lingjing-cloud`
- 后端 (8900) 保持 `0.0.0.0`（因systemd ExecStartPre 兼容性问题，暂不改动）

### 当前端口
| 端口 | 绑定 | 服务 |
|------|------|------|
| 8000 | 127.0.0.1 ✅ | Cloud |
| 8900 | 0.0.0.0 | 后端(Nginx代理,安全组防护) |
...
## 2026-07-07 (午间) — 运维检查 + 旧版兼容 ✅

### 日志排查
- Nginx 错误日志: 仅有旧版APK文件名404（已修复），无其他错误
- 后端: 运行7小时无异常
- PostgreSQL: 无报错
- fail2ban: 0当前失败

### 旧版兼容
- 外部请求在访问 `lingjing-v1.64.1.apk` 和 `lingjing-v1.18.9.apk`
- 创建软链接指向 `lingjing-1.73.188.apk`，避免404
- 所有旧链接均返回 200 OK
...
| 8900 | 后端 | 0.0.0.0 | ⚠️ 建议绑127.0.0.1 |
| 8000 | Cloud | 0.0.0.0 | ⚠️ 建议绑127.0.0.1 |
| 18900/22022 | SSH备用 | 127.0.0.1 | ✅ |

### 建议（非紧急）
- 8900/8000端口可改为127.0.0.1绑定（通过Nginx代理已足够）
- iptables/ufw均未启用，建议开启基础防火墙规则

---

## 2026-07-07 (午间) — 网站下载页更新 ✅

### 修改内容
- 下载页大小信息: `84MB` → `83MB | Android 7.0+ | arm64/armeabi/x86`
- 版本说明更新: 录音权限 + expo-av语音 + v2v3签名
- 新增 XZ 压缩版下载按钮 (29MB，`xz -d` 解压后安装)
- 服务器文件: `/var/www/html/spiritrealmz/index.html`
