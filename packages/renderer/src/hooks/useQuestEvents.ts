...
export function useQuestEvents(): void {
  useEffect(() => {
    ...
    const onFileBehaviorChanged = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (detail?.behavior) {
        cachedFileChangeBehavior = detail.behavior;
      }
    };
    window.addEventListener('quest:file-behavior-changed', onFileBehaviorChanged);
...
// Todo state sync
    ...
    return () => {
      ...
      unsubAsk();
      unsubConfirm();
      window.removeEventListener('quest:file-behavior-changed', onFileBehaviorChanged);
    };
  }, []);
}
