...
export const useAutomationStore = defineStore('automation', () => {
  ...
  async function loadList(): Promise<void> { loading.value = true; try { const res = await automationApi.list(); if (res.code === 0) list.value = res.data; } catch (e) { console.error("加载automation失败:", e); } finally { loading.value = false; } }
