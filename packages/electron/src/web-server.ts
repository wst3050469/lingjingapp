...
export function startWebServer(config: Partial<WebServerConfig> = {}): void {
  ...
  const authenticate = (req: Request, res: Response, next: NextFunction) => {
    ...
    const expectedBuf = Buffer.from(serverConfig.token, 'utf8');
  ...
  function handleWsCommand(ws: WebSocket, msg: any) {
    ...
    try {
      ...
      switch (channel) {
        ...
        case 'approval': handleApprovalCommand(ws, id, action, payload, db); break;
        case 'agent': handleAgentCommand(ws, id, action, payload, db); break;
        case 'task-summary': handleTaskSummaryCommand(ws, id, action, payload, db); break;
        default:
  ...
  function handleChatCommand(ws: WebSocket, id: string, action: string, payload: any, db: any) {
    if (action === 'list') {
      ...
      const mainWindow = getMainWindowFn!();
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('chat:answer-from-mobile', { sessionId: payload.sessionId, answer: payload.answer });
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { answered: true } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'No active window' }));
      }
    } else {
  ...
  function handleQuestCommand(ws: WebSocket, id: string, action: string, payload: any, db: any) {
    if (action === 'list') {
      ...
      // Restore quest session from SQLite checkpoint after reconnect
      const mainWindow = getMainWindowFn!();
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('quest:session-restore', { taskId: payload.taskId });
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { restored: true } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'No active window' }));
      }
    } else {
  ...
  function handleApprovalCommand(ws: WebSocket, id: string, action: string, payload: any, db: any) {
    if (action === 'list') {
      const rows = db.exec(`SELECT session_id, status, task_title FROM agent_sessions WHERE status IN ('running', 'paused') ORDER BY updated_at DESC LIMIT 20`);
      const pending = (rows[0]?.values || []).map((r: any) => ({ sessionId: r[0], status: r[1], taskTitle: r[2] }));
      ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { pending } }));
    } else if (action === 'approve') {
      const mainWindow = getMainWindowFn!();
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('approval:approve-from-mobile', { sessionId: payload.sessionId, approved: true });
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { approved: true } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'No active window' }));
      }
      ...
      const mainWindow = getMainWindowFn!();
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('approval:approve-from-mobile', { sessionId: payload.sessionId, approved: false });
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { rejected: true } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'No active window' }));
      }
    } else {
      ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'Unknown approval action: ' + action }));
    }
  }

  function handleAgentCommand(ws: WebSocket, id: string, action: string, payload: any, db: any) {
    if (action === 'list') {
      const rows = db.exec(`SELECT session_id, type, status, task_title, turn_count, updated_at FROM agent_sessions ORDER BY updated_at DESC LIMIT 30`);
      const agents = (rows[0]?.values || []).map((r: any) => ({ sessionId: r[0], type: r[1], status: r[2], taskTitle: r[3], turnCount: r[4], updatedAt: r[5] }));
      ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { agents } }));
    } else if (action === 'status') {
      const row = db.exec(`SELECT session_id, type, status, task_title, turn_count, updated_at FROM agent_sessions WHERE session_id = ?`, [payload.sessionId]);
      if (row[0]?.values.length) {
        const r = row[0].values[0];
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { sessionId: r[0], type: r[1], status: r[2], taskTitle: r[3], turnCount: r[4], updatedAt: r[5] } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'Agent not found' }));
      }
    } else if (action === 'switch') {
      const mainWindow = getMainWindowFn!();
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('agent:switch-from-mobile', { sessionId: payload.sessionId });
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { switched: true } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'No active window' }));
      }
    } else {
      ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'Unknown agent action: ' + action }));
    }
  }

  function handleTaskSummaryCommand(ws: WebSocket, id: string, action: string, payload: any, db: any) {
    if (action === 'subscribe') {
      ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { subscribed: true, channel: 'task-summary' } }));
    } else if (action === 'get') {
      const row = db.exec(`SELECT session_id, task_title, turn_count, status, updated_at FROM agent_sessions WHERE session_id = ?`, [payload.sessionId]);
      if (row[0]?.values.length) {
        const r = row[0].values[0];
        ws.send(JSON.stringify({ type: 'ack', id, success: true, data: { sessionId: r[0], title: r[1], currentStep: r[2], status: r[3], updatedAt: r[4] } }));
      } else {
        ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'Session not found' }));
      }
    } else {
      ws.send(JSON.stringify({ type: 'ack', id, success: false, error: 'Unknown task-summary action: ' + action }));
    }
  }
