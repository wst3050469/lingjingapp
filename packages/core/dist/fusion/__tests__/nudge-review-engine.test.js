...
describe('NudgeReviewEngine', () => {
    ...
    beforeEach(() => {
        handlers = {};
        eventBus = {
            subscribe: vi.fn((topic, handler) => { handlers[topic] = handler; return vi.fn(); }),
            publish: vi.fn(),
            unsubscribe: vi.fn(),
        };
        llmAdapter = {
    ...
    describe('startReview', () => {
        ...
        it('should return null without LLM adapter', async () => {
            ...
            noLLM.initialize(eventBus, null);
        ...
        it('should publish review:completed event', async () => {
            ...
            expect(eventBus.publish).toHaveBeenCalledWith('review:completed', expect.objectContaining({ originalMessageId: 'm1' }), 'NudgeReviewEngine');
        });
        it('should handle LLM errors gracefully', async () => {
            ...
            expect(result).toBeNull();
            expect(eventBus.publish).toHaveBeenCalledWith('review:failed', expect.objectContaining({ messageId: 'm1' }), 'NudgeReviewEngine');
        });
...
//# sourceMappingURL=nudge-review-engine.test.js.map