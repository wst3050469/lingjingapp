...
declare interface ElectronAPI {
  ...
  startup: {
    health: () => Promise<{
      version: string;
      platform: string;
      checkpoints: Array<{ id: string; name: string; timestamp: string; status: string; durationMs: number; metadata?: any }>;
      totalTimeMs: number;
      status: 'healthy' | 'degraded' | 'failed';
      warnings: Array<{ checkpointId: string; message: string }>;
    }>;
    stats: () => Promise<{ successRate: number; consecutiveFailures: number }>;
  };
  agent: {
  ...
  session: {
    ...
    list: (filter?: any) => Promise<{ success: boolean; sessions: any[] }>;
  ...
  memory: {
    ...
    pullFromCloud: () => Promise<{ success: boolean; count?: number; error?: string }>;
  };
  skills: {
    list: () => Promise<any[]>;
  ...
  cloud: {
    ...
    onRelayMessage: (callback: (data: any) => void) => () => void;
    relaySend: (opts: { type: string; payload: any; correlationId?: string }) => Promise<{ ok: boolean }>;
  ...
  // GAP-001: Debug API
  debug: {
    getConfig: () => Promise<{ version: string; configurations: Array<{ name: string; type: string; program?: string; cwd?: string; args?: string[]; runtimeExecutable?: string }> }>;
    saveConfig: (config: any) => Promise<{ success: boolean }>;
    start: (params: { config: any; workspacePath?: string }) => Promise<{ sessionId?: string; port?: number; devtoolsUrl?: string; error?: string }>;
    stop: (params: { sessionId: string }) => Promise<{ success?: boolean; error?: string }>;
    list: () => Promise<Array<{ id: string; configName: string; port: number; status: string; createdAt: number }>>;
    openDevtools: (params: { sessionId: string }) => Promise<{ success?: boolean; error?: string }>;
    onOutput: (callback: (event: { sessionId: string; data: string; stream: string }) => void) => () => void;
    onStatus: (callback: (event: { sessionId: string; status: string; port?: number; exitCode?: number; error?: string }) => void) => () => void;
  };
}
...
// (Stale module declarations removed — source files no longer exist in packages/core/src/)
