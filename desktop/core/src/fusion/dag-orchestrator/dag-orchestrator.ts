...
export class DAGOrchestrator {
  ...
  validateDAG(dag: DAGDefinition): { valid: boolean; error?: string } {
    if (!dag.id) return { valid: false, error: 'DAG id is required' };
    if (!dag.nodes || dag.nodes.length === 0) return { valid: false, error: 'DAG must have at least one node' };

    const nodeIds = new Set(dag.nodes.map((n) => n.taskId));
    const duplicates = dag.nodes.map((n) => n.taskId).filter((id, i, arr) => arr.indexOf(id) !== i);
    if (duplicates.length > 0) return { valid: false, error: `Duplicate node ids: ${duplicates.join(',')}` };

    for (const node of dag.nodes) {
      for (const dep of node.dependencies) {
        if (!nodeIds.has(dep)) return { valid: false, error: `Node ${node.taskId} depends on non-existent node ${dep}` };
      }
    }
    ...
    for (const nodeId of nodeIds) {
      if (!visited.has(nodeId)) {
        if (hasCycle(nodeId)) return { valid: false, error: 'DAG contains a cycle' };
      }
    }
  ...
  private evaluateCondition(condition: BranchCondition | undefined, context: Record<string, unknown>): boolean {
    if (!condition) return true;
    const value = condition.expression.split('.').reduce<unknown>(
  ...
  private async executeNodeWithRetry(
    ...
    retryPolicy: { maxRetries: number; retryDelay: number } | undefined,
  ): Promise<TaskResult> {
  ...
  async execute(dag: DAGDefinition, context: Record<string, unknown>): Promise<DAGResult> {
          ...
          if (settled.status === 'fulfilled') {
            ...
            if (result.status === 'failed') {
              ...
              this.eventBus?.publish(
                'dag:node_completed',
                { dagId: dag.id, taskId, status: result.status },
                'DAGOrchestrator',
              );
            }
          } else {
