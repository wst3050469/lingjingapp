...
export class BrowserService {
  ...
  async initialize(): Promise<void> {
    // GAP-004: Check if existing browser is still connected before reusing
    if (this.browser) {
      if (this.browser.isConnected()) {
        return;
      }
      // Browser died -- clean up stale references
      console.warn('[BrowserService] Existing browser disconnected, reinitializing');
      this.browser = null;
      this.context = null;
    ...
    try {
      ...
      this.browser = await chromium.launch({
        ...
        headless: process.env.CODEPILOT_BROWSER_HEADLESS === 'true',
      });

      // GAP-004: Listen for browser disconnect/crash and auto-reset state
      this.browser.on('disconnected', () => {
        console.warn('[BrowserService] Browser disconnected unexpectedly -- resetting state');
        this.browser = null;
        this.context = null;
        this.currentPage = null;
        this.state = {
