...
function AppContent() {
  ...
  // Save session snapshot before page unload (app close / reload)
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      ...
      if (chatStore.currentConversationId) {
        try {
          ...
          const title = chatStore.messages.find(m => m.role === 'user')?.content.slice(0, 50) || 'New Chat';
          const serialized = chatStore.messages.map(m => ({
            role: m.role,
            content: m.content,
            toolCalls: m.toolCalls,
          }));

          if (window.electronAPI?.conversation?.saveSync) {
            window.electronAPI.conversation.saveSync(userId, chatStore.currentConversationId, title, serialized);
            console.log('[App] Sync saved conversation to database');
          }
        } catch (err) {
          console.error('[App] Failed to sync save conversation:', err);
        }
      }

      event.returnValue = 'Saving conversation...';
    };
