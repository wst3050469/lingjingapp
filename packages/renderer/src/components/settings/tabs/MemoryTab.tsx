...
export function MemoryTab({ config, saveKey }: MemoryTabProps) {
  ...
  const handleCloudSearch = async () => {
    ...
    try {
      const results = await window.electronAPI.cloud.memories.search(cloudSearchQuery.trim());
  ...
  const handlePullFromCloud = async () => {
    ...
    try {
      const result = await window.electronAPI.memory.pullFromCloud();
      if (result.success) {
        console.log('Pulled ' + result.count + ' memories from cloud');
        loadMemories();
      }
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white/[0.02] border border-cp-border/30 rounded-xl p-4 mt-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-cp-text-dim/50 font-medium">云端记忆同步</p>
          <button
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="跨设备搜索云端记忆..."
            className="flex-1 bg-cp-bg border border-cp-border/50 rounded-lg px-3 py-1.5 text-sm text-cp-text outline-none focus:border-cp-accent placeholder:text-cp-text-dim/30"
          />
          <button
            onClick={handleCloudSearch}
            disabled={cloudSearching}
            className="text-xs px-3 py-1.5 rounded-md bg-white/[0.05] text-cp-text-dim hover:text-cp-text hover:bg-white/10 disabled:opacity-50 transition-colors"
          >
            {cloudSearching ? '搜索中...' : '云端搜索'}
          </button>
        </div>
        ...
        {cloudResults.length > 0 && (
          <div className="mt-2 space-y-1">
            {cloudResults.map((mem: any, i: number) => (
              <div key={mem.id || i} className="bg-cp-bg/50 rounded-lg px-3 py-2 text-xs text-cp-text-dim/70">
                <span className="font-medium text-cp-text-dim/90">{mem.title}</span>
                <span className="ml-2 text-cp-text-dim/40">{mem.content?.slice(0, 100)}</span>
              </div>
            ))}
          </div>
        )}
        {cloudResults.length === 0 && cloudSearchQuery && !cloudSearching && (
          <p className="mt-2 text-xs text-cp-text-dim/30">云端未找到匹配的记忆</p>
        )}
      </div>
