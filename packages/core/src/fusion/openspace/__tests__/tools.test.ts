...
describe('OpenSpace tools', () => {
  function createMockBridge(connected = true): OpenSpaceBridge {
    return {
  ...
  function createMockProcessManager(): OpenSpaceProcessManager {
    return {
      runState: 'running',
  ...
  describe('createOpenSpaceExecuteTool', () => {
    ...
    it('should handle script execution failure', async () => {
      const bridge = createMockBridge();
      (bridge.sendScript as any).mockResolvedValue({
        success: false,
    ...
    it('should handle bridge exception', async () => {
      const bridge = createMockBridge();
      (bridge.sendScript as any).mockRejectedValue(new Error('Connection lost'));
      const tool = createOpenSpaceExecuteTool(bridge);
    ...
    it('should accept custom language and timeout', async () => {
      ...
      await tool.execute(
        { script: 'print("hello")', language: 'python', timeout: 5000 },
        {} as any,
      );
  ...
  describe('createOpenSpaceQueryTool', () => {
    ...
    it('should return health info', async () => {
      ...
      const result = await tool.execute({ action: 'health' }, {} as any);
    ...
    it('should return scene context when bridge connected', async () => {
      ...
      const result = await tool.execute({ action: 'scene' }, {} as any);
    ...
    it('should handle scene query exception', async () => {
      const bridge = createMockBridge(true);
      (bridge.getSceneContext as any).mockRejectedValue(new Error('Bridge timeout'));
      const pm = createMockProcessManager();
      const tool = createOpenSpaceQueryTool(bridge, pm);
