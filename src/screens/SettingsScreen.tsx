...
export default function SettingsScreen() {
  ...
  function handleSave() {
    // BUGFIX: 验证必填字段，防止空值导致 new WebSocket() 同步异常崩溃
    if (!editingIp.trim()) {
      Alert.alert('提示', '请输入桌面端 IP 地址');
      return;
    }
    if (!editingToken.trim()) {
      Alert.alert('提示', '请输入配对 Token');
      return;
    }

    // BUGFIX: 清理 IP 格式（去除空格和协议前缀）
    const cleanIp = editingIp.trim().replace(/^https?:\/\//, '').replace(/\/$/, '');
    const lanUrl = `http://${cleanIp}:3001`;

    setToken(editingToken.trim());
    setLanIp(cleanIp);

    // BUGFIX: 包裹 try/catch 防止任何同步异常导致 App 退出
    try {
      api.configure({ baseUrl: lanUrl, token: editingToken.trim(), wsUrl: `ws://${cleanIp}:3001/ws` });
      api.connectWs();
    } catch (wsErr: any) {
      console.warn('WebSocket connect failed:', wsErr?.message);
      // 不崩溃 — WebSocket 失败仍可尝试 HTTP
    }

    // Try LAN first, then Cloud
    fetch(`${lanUrl}/api/status`, {
      headers: { Authorization: `Bearer ${editingToken.trim()}` },
    }).then(res => {
      if (res.ok) {
      ...
      try {
        api.configure({ baseUrl: cloudUrl, token: editingToken.trim(), wsUrl: 'wss://ide.zhejiangjinmo.com/ws' });
      } catch { /* ignore config error */ }
