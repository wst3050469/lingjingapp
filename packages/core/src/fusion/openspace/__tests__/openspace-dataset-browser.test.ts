...
import type { IFileSystem } from '../dataset-browser.js';
...
describe('OpenSpaceDatasetBrowser', () => {
  ...
  let mockBridge: any;
  let browser: OpenSpaceDatasetBrowser;
  ...
  describe('scanDatasets', () => {
    ...
    it('should scan directory and index supported files', async () => {
      ...
      expect(results.find((d: any) => d.name === 'star.fits')?.type).toBe('fits');
      expect(results.find((d: any) => d.name === 'planet.obj')?.type).toBe('mesh');
    });
    ...
    it('should scan subdirectories up to max depth', async () => {
      ...
      const galaxyEntry = results.find((d: any) => d.name === 'galaxy');
  ...
  describe('loadDataset', () => {
    ...
    it('should send load script via bridge', async () => {
      ...
      expect(mockBridge.sendScript).toHaveBeenCalledWith(
        expect.objectContaining({ script: expect.stringContaining('addSceneGraphNode') })
      );
    });
  ...
  describe('unloadDataset', () => {
    ...
    it('should send unload script via bridge', async () => {
      ...
      expect(mockBridge.sendScript).toHaveBeenCalledWith(
        expect.objectContaining({ script: expect.stringContaining('removeSceneGraphNode') })
      );
    });
