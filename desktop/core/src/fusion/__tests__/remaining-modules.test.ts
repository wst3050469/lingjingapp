...
describe('SkillSecurityLoader', () => {
  let loader: SkillSecurityLoader;
  ...
  describe('scanAndLoad', () => {
    ...
    it('should publish skill:blocked for dangerous content', async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        'skill:blocked',
        expect.objectContaining({ skillPath: '/test.skill.md' }),
        'SkillSecurityLoader',
      );
    });
  });
...
describe('ConnectorHubAdapter', () => {
  let hub: ConnectorHubAdapter;

  beforeEach(() => {
    hub = new ConnectorHubAdapter();
  });

  function makeConnector(name: string, type: string) {
    return {
      name,
      type,
