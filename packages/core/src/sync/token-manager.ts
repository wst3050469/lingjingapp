...
interface TokenManagerConfig {
  autoRefresh: boolean;
  refreshThreshold: number;  // ms before expiry to trigger refresh
  maxTokenAge: number;        // default token age for refreshed tokens
}
...
export class TokenManager {
  ...
  async saveAccount(account: GitHubAccount): Promise<void> {
    const key = `github_account_${account.id}`;
    this.storage.saveToken(key, JSON.stringify(account));
  }

  async loadAccount(accountId: string): Promise<GitHubAccount | null> {
    ...
    try {
      return JSON.parse(data) as GitHubAccount;
    } catch {
      return null;
    }
  }

  async loadAllAccounts(): Promise<GitHubAccount[]> {
    ...
    for (const key of keys) {
      ...
      if (data) {
        try {
          accounts.push(JSON.parse(data) as GitHubAccount);
        } catch {
          // skip corrupted data
        }
      }
  ...
  async getActiveToken(accountId: string): Promise<string | null> {
  ...
  async refreshToken(account: GitHubAccount): Promise<GitHubAccount | null> {
