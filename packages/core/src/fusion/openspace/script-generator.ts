...
const HIGH_RISK_SUGGESTIONS: Record<string, string[]> = {
...
function defaultParamExtractor(input: string, template: ScriptTemplate): Record<string, string | number> {
...
export class OpenSpaceScriptGenerator {
  ...
  private paramExtractor: TemplateParamExtractor;
  private generationHistory: Array<{ input: string; result: GenerationResult; timestamp: number }> = [];
  ...
  async generate(naturalLanguage: string, sceneContext?: SceneContext): Promise<GenerationResult> {
  ...
  private async generateFromLLM(
    ...
    try {
      ...
      const errorResult: GenerationResult = {
        script: `openspace.screenlog("Generation error: ${(err as Error).message.replace(/"/g, '\\"')}")`,
        language: 'lua',
        source: 'llm',
        reviewResult: { passed: true, riskLevel: 'none', violations: [] },
        confidence: 0.0,
        error: `LLM generation failed: ${(err as Error).message}`,
      };
      this.addToHistory(input, errorResult);
      logger.error(`[ScriptGenerator] LLM error: ${(err as Error).message}`);
      return errorResult;
    }
  }

  private buildUserPrompt(input: string, sceneContext?: SceneContext): string {
  ...
  private sanitizeLLMOutput(raw: string): string {
    let script = raw.trim();
  ...
  private addToHistory(input: string, result: GenerationResult): void {
  ...
  getHistory(): ReadonlyArray<{ input: string; result: GenerationResult; timestamp: number }> {
