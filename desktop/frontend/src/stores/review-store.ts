...
export const useReviewStore = create<ReviewState>((set) => ({
  ...
  executeReview: async (projectPath, diff, filePath, lang, prId, branch, sha) => {
    set({ loading: true });
    try {
      const report = await window.electronAPI.invoke('review:execute', projectPath, diff, filePath, lang, prId, branch, sha);
  ...
  getReport: async (projectPath, reportId) => {
    const report = await window.electronAPI.invoke('review:getReport', projectPath, reportId);
    set({ currentReport: report });
  },

  listReports: async (projectPath, filter) => {
    const reports = await window.electronAPI.invoke('review:listReports', projectPath, filter);
    set({ reportHistory: reports });
  },

  applyFix: async (projectPath, reportId, idx) => {
    await window.electronAPI.invoke('review:applyFix', projectPath, reportId, idx);
  },

  loadRules: async (projectPath) => {
    const rules = await window.electronAPI.invoke('review:listRules', projectPath);
    set({ customRules: rules });
  },

  saveRule: async (projectPath, rule) => {
    await window.electronAPI.invoke('review:saveRule', projectPath, rule);
  },

  deleteRule: async (projectPath, ruleId) => {
    await window.electronAPI.invoke('review:deleteRule', projectPath, ruleId);
  },
}));
