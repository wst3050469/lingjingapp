...
import { codepilotResolvePlugin } from './codepilot-resolve-plugin.mjs';
...
const EXTERNAL = [
  ...
  'playwright',
  'playwright-core',
  // @codepilot/core — now BUNDLED via codepilotResolvePlugin (subpath resolution fix)
...
// electron-builder packages files from packages/electron/ using "files": ["renderer/**/*"].
// The Vite build output is at packages/renderer/dist/, which needs to be copied
// to packages/electron/renderer/ so electron-builder includes it in the ASAR.
// Without this step, only index.html is packaged (no assets/) → blank page at runtime.
const RENDERER_SRC = resolve(root, '../renderer/dist');
...
try {
  await esbuild.build({
    ...
    format: 'cjs',
    external: EXTERNAL,
    plugins: [importMetaPlugin(), codepilotResolvePlugin()],
    sourcemap: true,
    minify: false,
    logLevel: 'info',
  ...
  function syncExternalNodeModules() {
    ...
    // Special: Ensure @codepilot/core has real files (no pnpm links).
    // Copy ONLY dist + package.json from workspace packages/core.
    // Do NOT copy node_modules — pnpm nested symlinks break when relocated.
    const WS_CORE = join(root, "..", "..", "packages", "core");
    const CD_DST = join(SRC_NM, "@codepilot");
    const CORE_DST = join(CD_DST, "core");
    if (existsSync(CD_DST)) {
      console.log("[build-main] Removing existing @codepilot link/copy");
      rmSync(CD_DST, { recursive: true, force: true });
    }
    if (existsSync(WS_CORE)) {
      mkdirSync(CORE_DST, { recursive: true });
      cpSync(join(WS_CORE, "dist"), join(CORE_DST, "dist"), { recursive: true, force: true });
      copyFileSync(join(WS_CORE, "package.json"), join(CORE_DST, "package.json"));
      console.log("[build-main] Copied @codepilot/core (dist + package.json) from workspace");
    } else {
      console.warn("[build-main] Workspace packages/core not found at " + "WS_CORE");
    }
