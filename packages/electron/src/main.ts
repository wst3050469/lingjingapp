// v1.72.6f: Write boot log immediately on process start, before ANY import
import { writeFileSync, mkdirSync } from 'node:fs';
import { join } from 'node:path';
import { homedir } from 'node:os';
import { randomBytes } from 'node:crypto';
try {
  const bootDir = join(homedir(), '.lingjing');
  mkdirSync(bootDir, { recursive: true });
  writeFileSync(join(bootDir, 'boot.log'), 'process:started\n');
} catch {}
...
import { registerBugIpc } from './ipc/bug-ipc.js';
import { registerSessionCheckpointIpc } from './ipc/session-checkpoint-ipc.js';
import { registerMarketplaceIpc } from './ipc/marketplace-ipc.js';
import { registerPushIpc } from './ipc/push-ipc.js';
import { registerHwSkillIpc } from './ipc/hw-skill-ipc.js';
import { registerHwAiIpc } from './ipc/hw-ai-ipc.js';
import { registerSkillReviewIpc } from './ipc/skill-review-ipc.js';
import { registerDebugIpc } from './ipc/debug-ipc.js';
...
import { startupHealthCheck } from './startup-health-check.js';
import { validateWindowBounds, recordWindowCreateFailure, resetWindowCreateFailCount } from './window-utils.js';
import { cleanupStaleProcesses } from './stale-process-cleaner.js';
import { recordStartup, getConsecutiveFailures } from './startup-stats.js';
import { checkRollbackNeeded, executeRollback } from './safe-rollback.js';
import * as fusionMod from '@codepilot/core/fusion';
import * as coreMod from '@codepilot/core';
...
function createWindow(): void {
  ...
  let savedBounds: Partial<{ width: number; height: number; x: number; y: number }> | undefined;
  try {
    const configPath = join(homedir(), '.lingjing', 'config.json');
    if (existsSync(configPath)) {
      const cfg = JSON.parse(readFileSync(configPath, 'utf8'));
      if (cfg.windowBounds) savedBounds = cfg.windowBounds;
    }
  } catch { /* ignore */ }
  ...
  try {
    ...
    resetWindowCreateFailCount();
    // v1.72.11: Log GPU feature status for diagnosing no-window issues
    try {
      const gpuInfo = app.getGPUFeatureStatus();
      console.log('[Main] Window created successfully. GPU feature status:', JSON.stringify(gpuInfo));
      if (gpuInfo && typeof gpuInfo === 'object') {
        const blockedFeatures = Object.entries(gpuInfo).filter(([, v]) => v !== 'enabled');
        if (blockedFeatures.length > 0) {
          console.warn('[Main] ⚠️ Some GPU features blocked:', blockedFeatures.map(([k,v]) => `${k}=${v}`).join(', '));
        }
      }
    } catch (gpuErr) {
      console.warn('[Main] Could not query GPU feature status:', gpuErr);
    }
  } catch (err) {
    const degraded = recordWindowCreateFailure();
    if (degraded) {
      console.warn('[Main] Attempting degraded window creation (no saved bounds, safe defaults)');
      try {
        mainWindow = new BrowserWindow({
          width: 1400, height: 900, minWidth: 800, minHeight: 600,
          title: '灵境', backgroundColor: '#1e1e1e', show: true,
          autoHideMenuBar: true,
          webPreferences: {
            preload: join(__dirname, 'preload.js'),
        ...
        resetWindowCreateFailCount();
        console.log('[Main] Degraded window created successfully');
      } catch (err2) {
        console.error('[Main] Degraded window creation also failed:', err2);
        throw err2;
      }
    } else {
      throw err;
  ...
  mainWindow.on('close', () => {
    try {
      if (mainWindow && !mainWindow.isDestroyed()) {
        ...
        const configPath = join(homedir(), '.lingjing', 'config.json');
        let cfg: any = {};
        try { if (existsSync(configPath)) cfg = JSON.parse(readFileSync(configPath, 'utf8')); } catch { /* ignore */ }
        cfg.windowBounds = { width: b.width, height: b.height, x: b.x, y: b.y };
  ...
  if (!existsSync(preloadPath)) {
    console.error('[Main] CRITICAL: preload.js not found at:', preloadPath);
    const preloadErrorHtml = `<html><body style="background:#1e1e1e;color:#cd3131;font-family:monospace;padding:40px">
      <h2>灵境 IDE — preload.js 缺失</h2>
      <p>文件路径: ${preloadPath}</p>
      <p>__dirname: ${__dirname}</p>
      <p>app.isPackaged: ${app.isPackaged}</p>
      <hr><p>请重新安装应用或联系技术支持</p>
      </body></html>`;
    mainWindow?.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(preloadErrorHtml)}`);
    return;
  }
  console.log('[Main] Preload exists:', true);

  // ── Renderer failure protection: showTimeout ──
  // Since show:true, the window appears immediately. This timeout
  // detects if the renderer process hung before producing any content.
  let rendererReady = false;
  const showTimeout = setTimeout(() => {
    if (mainWindow && !mainWindow.isDestroyed() && !rendererReady) {
      console.warn('[Main] Renderer not ready after 10s — reloading');
      mainWindow.webContents.reload();
    }
  }, 10000);

  let whiteScreenTimeout = setTimeout(() => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.executeJavaScript('document.body.innerHTML.length').then((len: number) => {
        if (len < 50) {
          console.warn('[Main] White screen detected after 30s — reloading');
          startupHealthCheck.record('CP7', 'renderer:white-screen', 'failed', { bodyLength: len });
          mainWindow?.webContents.reload();
        }
      }).catch(() => { /* ignore */ });
    }
  ...
  // Track when renderer is fully loaded
  mainWindow.webContents.on('did-finish-load', () => {
    rendererReady = true;
    clearTimeout(showTimeout);
    clearTimeout(whiteScreenTimeout);
    startupHealthCheck.record('CP7', 'renderer:did-finish-load', 'passed', { url: mainWindow?.webContents.getURL() });
  });
  ...
  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL) => {
    ...
    if (failLoadRetries < MAX_FAIL_RETRIES) {
      failLoadRetries++;
      const backoffMs = Math.pow(2, failLoadRetries - 1) * 1000;
      console.warn(`[Main] Retrying load in ${backoffMs}ms (attempt ${failLoadRetries}/${MAX_FAIL_RETRIES})...`);
      setTimeout(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.reload();
        }
      ...
      console.error('[Main] Max fail-load retries exceeded — showing diagnostic error page');
      if (mainWindow && !mainWindow.isDestroyed()) {
        const diagHtml = `<html><body style="background:#1e1e1e;color:#cd3131;font-family:monospace;padding:40px">
          <h2>灵境 IDE — 渲染进程加载失败</h2>
          <p>错误代码: ${errorCode}</p>
          <p>错误描述: ${errorDescription}</p>
          <p>请求URL: ${validatedURL}</p>
          <p>重试次数: ${failLoadRetries}</p>
          <hr><p>排查建议：<br>1. 检查renderer构建产物是否存在<br>2. 检查磁盘空间和文件权限<br>3. 删除 ~/.lingjing 目录后重试</p>
          </body></html>`;
        mainWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(diagHtml)}`);
      }
    }
  });

  // ── Renderer failure protection layer 2.5: render-process-gone ──
  // Catches renderer process crashes (GPU crash, OOM, etc.)
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[Main] Renderer process gone — reason:', details.reason, 'exitCode:', details.exitCode);
    if (mainWindow && !mainWindow.isDestroyed()) {
      // Show the window if it's hidden, then reload to attempt recovery
      mainWindow.show();
      if (details.reason === 'crashed' || details.reason === 'oom') {
        console.warn('[Main] Renderer crashed/OOM — reloading in 2s');
        setTimeout(() => {
          if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.reload();
          }
  ...
  if (IS_DEV) {
    ...
    // Attempt to load the renderer. If it fails, show a visible error page
    // so the user can diagnose the problem (instead of a silent blank window).
    mainWindow.loadFile(rendererPath).catch((err) => {
      console.error('[Main] Failed to load renderer:', err.message);
      // Show a visible error HTML page so the window doesn't stay blank
      const errorHtml = `<html><body style="background:#1e1e1e;color:#cd3131;font-family:monospace;padding:40px">
        <h2>灵境 IDE — 渲染进程加载失败</h2>
        <p>文件路径: ${rendererPath}</p>
        <p>错误: ${err.message}</p>
        <p>文件存在: ${existsSync(rendererPath)}</p>
        <hr><p>请联系技术支持并提供上述信息</p>
        </body></html>`;
...
async function registerAppIpc(): Promise<void> {
  ...
  ipcMain.handle('config:get', async () => {
    // @ts-ignore -- getModelContextWindow is a runtime extension not in core types
    const { loadConfig, getModelContextWindow } = coreMod;
  ...
  // DEF-014: 配置写入串行化，防止并发竞态
  const { AsyncMutex } = await import('./utils/async-mutex.js');
  ...
  ipcMain.handle('tools:list', async () => {
    const { createDefaultRegistry } = coreMod;
...
async function bootstrap(): Promise<void> {
  // v1.72.6: Boot diagnostic — write every startup step to boot.log
  // so we can identify exactly where the process gets stuck.
  const bootSteps: string[] = [];
  function bootLog(s: string) {
    bootSteps.push(new Date().toISOString() + ' ' + s);
    try {
      const fs = require('node:fs'), p = require('node:path'), o = require('node:os');
      const d = p.join(o.homedir(), '.lingjing');
      fs.mkdirSync(d, { recursive: true });
      fs.writeFileSync(p.join(d, 'boot.log'), bootSteps.join('\n'));
    } catch {}
  }
  bootLog('bootstrap:start');
  startupHealthCheck.setBootLogFn(bootLog);
  startupHealthCheck.record('CP1', 'process:started', 'passed');
  startupHealthCheck.record('CP2', 'app.whenReady', 'passed');
  ...
  // DEF-017: CSP nonce策略，移除unsafe-inline
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    const nonce = randomBytes(16).toString('base64');
    const responseHeaders = {
      ...details.responseHeaders,
      'Content-Security-Policy': [
        `default-src 'self'; img-src 'self' data: blob: https:; script-src 'self' blob: 'nonce-${nonce}'; style-src 'self' 'unsafe-inline'; font-src 'self' data:; connect-src 'self' https://ide.zhejiangjinmo.com http://localhost:* ws://localhost:* wss://ide.zhejiangjinmo.com; worker-src 'self' blob:; frame-src 'self' http://localhost:*;`,
      ],
    };
  ...
  try {
    ...
    startupHealthCheck.record('CP3', 'initDatabase', 'passed', { durationMs: Date.now() - cp3Start });
    console.error('[Main] Database initialized successfully');
  } catch (err) {
    dbInitStatus = 'failed';
    dbInitError = err as Error;
    startupHealthCheck.record('CP3', 'initDatabase', 'failed', { error: String(err) });
    console.error('[Main] Failed to initialize database:', err);
    // Show error dialog and quit - database is critical for ALL features
    dialog.showErrorBox(
      '数据库初始化失败',
      `灵境无法初始化本地数据库，应用无法继续运行。\n\n` +
      `错误: ${(err as Error).message || String(err)}\n\n` +
      `数据库路径: ~/.lingjing/lingjing.db\n` +
      `请检查磁盘空间、写入权限，或删除 ~/.lingjing/lingjing.db 后重试。`
    );
    app.quit();
    return;
  }
  ...
  // v1.72.4: Wrapped in try/catch to prevent ANY exception from silently killing the window
  try {
    createWindow();
    startupHealthCheck.record('CP4', 'createWindow', 'passed');
  } catch (err) {
    console.error('[Main] createWindow threw:', err);
    // Ultimate fallback: create a bare-minimum window with error message
    try {
      mainWindow = new BrowserWindow({
        width: 800, height: 600, show: true, title: '灵境 - 错误',
        webPreferences: { sandbox: false, contextIsolation: false, nodeIntegration: true },
      });
      mainWindow.loadURL('data:text/html,<h1 style="color:red;font-family:monospace">启动失败</h1><pre>' + String(err) + '</pre>');
    } catch (err2) {
      dialog.showErrorBox('严重错误', '无法创建窗口: ' + String(err2));
      app.quit();
      return;
    }
  }
    ...
    mainWindow.webContents.on('did-finish-load', () => {
      if (mainWindow && !mainWindow.isDestroyed()) {
        ...
        mainWindow.webContents.send('app:db-status', {
          status: dbInitStatus,
          error: (dbInitError as Error | null)?.message,
          stack: (dbInitError as Error | null)?.stack,
        });
      }
    });
  }
  ...
  // Database Health Check IPC (registered early, no window dependency)
  try {
    ipcMain.handle('db:health', async () => {
      try {
        ...
        const criticalTables = ['quest_tasks', 'quest_messages', 'conversations', 'messages', 'users'];
        ...
        return {
          ...
          dbPath: (await import('node:os').then(m => m.homedir())) + '/.lingjing/lingjing.db',
        };
      } catch (err) {
        return {
          healthy: false,
          error: err instanceof Error ? err.message : String(err),
          suggestion: 'Database not initialized. Delete ~/.lingjing/lingjing.db and restart.',
        };
      }
    });
    console.log('[Main] db:health handler registered');
  } catch (err) {
    console.error('[Main] db:health registration failed:', err);
  }

  try {
    ipcMain.handle('startup:health', async () => {
      return startupHealthCheck.getReport();
    });
    ipcMain.handle('startup:stats', async () => {
      const { getSuccessRate } = await import('./startup-stats.js');
      return { successRate: getSuccessRate(), consecutiveFailures: getConsecutiveFailures() };
    });
    console.log('[Main] startup:health and startup:stats handlers registered');
  } catch (err) {
    console.error('[Main] startup IPC registration failed:', err);
  }
...
// @ts-expect-error - TS2345: BrowserWindow | null not assignable to BrowserWindow
  ...
  // IDE Enhancement IPC modules (bug fix, session checkpoint, marketplace, push)
  try {
    registerBugIpc();
    registerSessionCheckpointIpc();
    registerMarketplaceIpc();
    registerPushIpc();
    registerHwSkillIpc();
    registerHwAiIpc();
    registerSkillReviewIpc();
    console.log('[Main] IDE Enhancement IPC registered');
  } catch (err) {
    console.warn('[Main] IDE Enhancement IPC registration:', err);
  }
  ...
  startupHealthCheck.record('CP5', 'PhaseA_IPC', 'passed');
  // These handlers require a valid BrowserWindow instance and will be
  // skipped if the window failed to create.
  if (mainWindow) {
    try { registerAgentIpc(mainWindow); } catch (err) { console.error("[Main] registerAgentIpc:", err); }
    try { registerFsIpc(mainWindow, () => workspacePath); } catch (err) { console.error("[Main] registerFsIpc:", err); }
    try { registerTerminalIpc(mainWindow); } catch (err) { console.error("[Main] registerTerminalIpc:", err); }
    try { registerGitIpc(() => workspacePath); } catch (err) { console.error("[Main] registerGitIpc:", err); }
    try { registerSkillsIpc(() => workspacePath); } catch (err) { console.error("[Main] registerSkillsIpc:", err); }
    try { registerIndexingIpc(() => workspacePath, mainWindow); } catch (err) { console.error("[Main] registerIndexingIpc:", err); }
    try { registerDiagnosticsIpc(mainWindow, () => workspacePath); } catch (err) { console.error("[Main] registerDiagnosticsIpc:", err); }
    ...
    try { registerWikiIpc(mainWindow, () => workspacePath); } catch (err) { console.error("[Main] registerWikiIpc:", err); }
    try { registerBrowserIpc(mainWindow); } catch (err) { console.error("[Main] registerBrowserIpc:", err); }
    try { registerDebugIpc(mainWindow); } catch (err) { console.error("[Main] registerDebugIpc:", err); }
    try { registerPlanIpc(mainWindow); } catch (err) { console.error("[Main] registerPlanIpc:", err); }
    try { registerContextIpc(() => workspacePath); } catch (err) { console.error("[Main] registerContextIpc:", err); }
    try { setSshTerminalChangeCallback((sshTerminalId) => {
      setSshTerminalId(sshTerminalId);
      setQuestSshTerminalId(sshTerminalId);
    });
    registerSshIpc(mainWindow); } catch (err) { console.error("[Main] setSshTerminalChangeCallback:", err); }
    ...
    try {
      ...
      try {
        ...
        const {
          ...
          // SqliteVectorAdapter not in fusion dist
        ...
        if (FusionInitializer) {
          ...
          try {
            const db = getDatabase();
            // SqliteVectorAdapter not available, fall back to InMemoryVectorAdapter
            const inMemAdapter = new InMemoryVectorAdapter();
            const vm = new VectorMemoryStore(DEFAULT_VECTOR_MEMORY_CONFIG, inMemAdapter);
            // Inject real embedding function using createEmbeddingService
            try {
              // @ts-expect-error - dynamic import path resolved by esbuild
              const { createEmbeddingService } = await import('../services/embedding-service.js');
              const { loadConfig: loadCoreConfig } = coreMod;
              ...
              // @ts-ignore - setEmbedFn is a custom extension not in dist types
            ...
            console.log(`[Main] Fusion VectorMemory created (InMemoryAdapter)`);
          ...
          try {
            // @ts-ignore - callback type mismatch between Promise<string> and Promise<{success,error}>
            const dag = new DAGOrchestrator(async () => ({ success: false, error: 'no-executor' })); init.setDAGOrchestrator(dag); modules.dagOrchestrator = dag; console.log('[Main] Fusion DAGOrchestrator created'); } catch (e) { console.warn('[Main] DAGOrchestrator failed:', e); }
          try {
            // @ts-ignore - constructor expects (callback, config) but we pass (config, callback)
            const ma = new MultiAgentExecutor(DEFAULT_MULTI_AGENT_CONFIG, async () => ({ success: false, error: 'no-executor' })); init.setMultiAgent(ma); modules.multiAgent = ma; console.log('[Main] Fusion MultiAgent created'); } catch (e) { console.warn('[Main] MultiAgent failed:', e); }
          ...
          try {
            ...
            if (integ && integ.registerFusionTools) {
              // @ts-ignore - registerFusionTools expects 3 args; called with 0 for auto-discovery
          ...
          try {
            ...
            if (integ && integ.setupMemoryLinkages) {
              // @ts-ignore - setupMemoryLinkages expects (eventBus, deps) but called with {db}
    ...
    try {
      ...
      startupHealthCheck.record('CP6', 'PhaseB_IPC', missing.length === 0 ? 'passed' : 'failed',
        { missingCount: missing.length, missing: missing.slice(0, 10) });
    } catch (err) {
      console.error('[Main] IPC verification failed:', err);
      startupHealthCheck.record('CP6', 'PhaseB_IPC', 'failed', { error: String(err) });
    }
  }

  // ── Renderer failure protection layer 3: Force-show window if still hidden ──
  // At this point all IPC handlers are registered and the bootstrap is complete.
  // If the window is still hidden (ready-to-show didn't fire), force-show it now.
  // Also hide the 10s timeout — we already waited for all Phase B to complete.
  if (mainWindow && !mainWindow.isDestroyed() && !mainWindow.isVisible()) {
    console.warn('[Main] Window still hidden after bootstrap — forcing show');
    mainWindow.show();
    mainWindow.focus();
  }
  ...
  startupHealthCheck.record('CP9', 'loadWorkspace', 'passed', { path: loadedWorkspace });
  ...
  try {
    ...
    startupHealthCheck.record('CP8', 'initAgent', 'passed');
  } catch (err) {
    console.error('[Main] Failed to initialize agent:', err);
    startupHealthCheck.record('CP8', 'initAgent', 'failed', { error: String(err) });
  }

  const failedCheckpoints = startupHealthCheck.getFailedCheckpoints();
  const firstTs = startupHealthCheck.getReport().checkpoints[0]?.timestamp;
  const totalTimeMs = firstTs ? Date.now() - new Date(firstTs).getTime() : 0;
  recordStartup(true, totalTimeMs, failedCheckpoints);

  if (failedCheckpoints.length > 0 && checkRollbackNeeded()) {
    await executeRollback(mainWindow);
    return;
  }
}
...
process.on('uncaughtException', (error) => {
  ...
  // v1.72.5: Show error dialog instead of silently crashing.
  // Previous behavior (process.exit(1)) left users with no feedback.
  try {
    dialog.showErrorBox(
      '灵境 - 未捕获异常',
    ...
    // dialog may fail if app is not ready
  }
...
process.on('unhandledRejection', (reason) => {
  ...
  // v1.72.5: Also show error for unhandled rejections
  try {
    dialog.showErrorBox(
      '灵境 - 未处理错误',
...
// v1.72.10: Remove --disable-gpu.
// v1.72.11: Also removed from window-utils.ts fallback (recordWindowCreateFailure).
//
// --disable-gpu blocks the entire GPU process, which on some Windows
// and Linux machines prevents even basic window compositing (no ANGLE,
// no Skia GPU backend, and the CPU fallback may also fail). The result
// is a "no window" bug where BrowserWindow exists but draws nothing.
//
// The correct approach is to let Electron/Chromium auto-detect GPU
// capabilities and fall back naturally. If specific users need GPU
// workarounds, they can pass --disable-gpu via command line or
// environment variables.
//
// v1.72.8 kept --disable-gpu believing it was needed for stability,
// but our testing showed it causes rather than prevents rendering
// failures on a significant subset of hardware.
//
// v1.72.10 originally removed it from main.ts, but window-utils.ts
// had a fallback mechanism that re-enabled it after 3 consecutive
// window creation failures — forming a "dormant timebomb" that could
// re-trigger the exact same no-window bug. v1.72.11 removes that too.

// v1.72.20: Call stale process cleanup (single-instance lock + temp/lock files)
// Must be called BEFORE app.whenReady() as per Electron best practices.
cleanupStaleProcesses();

app.whenReady().then(bootstrap).catch((err) => {
  console.error('[Main] bootstrap crashed:', err);
  // Record startup failure for rollback analysis
  (async () => {
    // Import dynamically to avoid circular dependency before module init
    const { recordStartup } = await import('./startup-stats.js');
    const { checkRollbackNeeded, executeRollback } = await import('./safe-rollback.js');
    recordStartup(false, 0, ['CP1']);
    if (checkRollbackNeeded()) {
      await executeRollback(null);
      return;
    }
    await app.whenReady();
    dialog.showErrorBox(
