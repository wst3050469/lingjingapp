...
export const useInvoiceStore = defineStore('invoices', () => {
  ...
  async function loadList(params?: Record<string, any>): Promise<void> { loading.value = true; try { const res = await invoiceApi.list(params); if (res.code === 0) list.value = res.data; } finally { loading.value = false; } }
