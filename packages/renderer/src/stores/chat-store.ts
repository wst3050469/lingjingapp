...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  addMessage: (msg) => {
    // BUG-023: Cap messages at MAX_VISIBLE_MESSAGES to prevent unbounded memory growth.
    // Tool result messages with very large content are truncated to MAX_TOOL_RESULT_LENGTH.
    const MAX_VISIBLE_MESSAGES = 200;
    const MAX_TOOL_RESULT_LENGTH = 10000;

    // Truncate large tool result content
    let truncatedMsg = msg;
    if (msg.role === 'tool' && msg.toolCalls?.[0]?.result?.content) {
      const resultContent = msg.toolCalls[0].result.content;
        ...
        truncatedMsg = {
          ...
          toolCalls: [{
            ...
            result: {
              ...
              content: resultContent.slice(0, MAX_TOOL_RESULT_LENGTH)
                + `\n...(truncated ${resultContent.length - MAX_TOOL_RESULT_LENGTH} chars)`,
            },
          }],
        };
      }
    ...
    set((state) => {
      const newMessages = [...state.messages, truncatedMsg];
      // Trim oldest messages if exceeding limit (keep user + assistant pairs together)
      if (newMessages.length > MAX_VISIBLE_MESSAGES) {
        const excess = newMessages.length - MAX_VISIBLE_MESSAGES;
        // Skip first user message to preserve conversation start context
        const firstUserIdx = newMessages.findIndex(m => m.role === 'user');
        const trimStart = Math.max(firstUserIdx, excess);
        return { messages: newMessages.slice(trimStart) };
      }
    ...
    if (snapshot?.conversationId) {
      try {
        ...
        if (loadedMsgs.length > 0) {
          ...
          // Notify main process to restore AgentSession from SQLite checkpoint
          try { (window.electronAPI as any)?.session?.restore?.({ sessionId: snapshot.conversationId }); } catch {}
    ...
    if (conversations.length > 0 && !get().currentConversationId) {
      ...
      try {
        ...
        if (get().messages.length === 0) {
          ...
          // Notify main process to restore AgentSession from SQLite
          try { (window.electronAPI as any)?.session?.restore?.({ sessionId: mostRecent.id }); } catch {}
        }
      } catch (err) {
