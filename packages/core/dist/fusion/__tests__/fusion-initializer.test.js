...
describe('FusionInitializer', () => {
    ...
    describe('setters', () => {
        it('should accept all dependencies via setters', () => {
            ...
            initializer.setVectorMemory({ healthCheck: vi.fn(() => ({ healthy: true })) });
            initializer.setReviewEngine({ healthCheck: vi.fn(() => ({ healthy: true })) });
            initializer.setSecurityScanner({ scan: vi.fn() });
            initializer.setTraceHarvester({ healthCheck: vi.fn(() => ({ healthy: true })) });
            initializer.setDAGOrchestrator({ healthCheck: vi.fn(() => ({ healthy: true })), validateDAG: vi.fn(), buildExecutionPlan: vi.fn(), execute: vi.fn() });
            initializer.setMultiAgent({ healthCheck: vi.fn(() => ({ healthy: true })), execute: vi.fn() });
            initializer.setModelRouter({ healthCheck: vi.fn(() => ({ healthy: true, rulesCount: 0 })), route: vi.fn(), evaluateTaskFeatures: vi.fn(), addRule: vi.fn(), removeRule: vi.fn(), getRules: vi.fn() });
            initializer.setNLCron({ healthCheck: vi.fn(() => ({ healthy: true })), scheduleFromNL: vi.fn(), listSchedules: vi.fn(), cancelSchedule: vi.fn(), previewCron: vi.fn() });
            initializer.setUserModeler({ healthCheck: vi.fn(() => ({ healthy: true })), updateUserModel: vi.fn(), getCurrentModel: vi.fn(), triggerReflection: vi.fn() });
    ...
    describe('toggleModule', () => {
        ...
        it('should not throw for unknown module', () => {
            expect(() => initializer.toggleModule('eventBus', true)).not.toThrow();
        });
    });
...
//# sourceMappingURL=fusion-initializer.test.js.map