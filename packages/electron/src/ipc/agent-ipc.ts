...
import { searchMemories, extractKeywords } from '../services/memory-service.js';
import { writeExecutionTrace } from '../services/execution-trace-writer.js';
...
async function searchExpertMemory(query: string, projectPath: string): Promise<string> {
  // Use the shared searchMemories() which searches ALL categories (not just expert-learning)
  // This ensures cross-session persistence: preferences, projects, workflows, issues, knowledge all retrieved
  const keywords = extractKeywords(query);
  if (!keywords) return '';
  return searchMemories(keywords, projectPath, getDatabase);
}
...
export function registerAgentIpc(mainWindow: BrowserWindow): void {
  ...
  ipcMain.handle('agent:run', async (event: IpcMainInvokeEvent, { message, mode, images, conversationMessages, conversationId }: {
    ...
    // BUG-018: Helper to wrap a tool's execute method with execution tracing
    const withTrace = (tool: Tool): Tool => {
      const originalExecute = tool.execute.bind(tool);
      const sessionId = conversationId || 'chat';
      return {
        ...tool,
        async execute(params: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    ...
    if (vectorMem && mode !== 'ask') {
      try {
        // @ts-ignore - fusionMod may be dynamically imported from @codepilot/core
        const { createRememberVectorTool, createRecallVectorTool } = (globalThis as any).fusionMod || {};
        const rememberTool = createRememberVectorTool(vectorMem);
        const recallTool = createRecallVectorTool(vectorMem);
        runTools.register(withTrace(rememberTool as any));
        runTools.register(withTrace(recallTool as any));
      } catch (e) {
...
Only save genuinely useful, non-trivial information. Do NOT save obvious or temporary details.`;
    ...
    // Session persistence: track this agent run via AgentSessionManager
    if (conversationId) {
    ...
    try {
      ...
      // Session persistence: checkpoint after successful run
      if (conversationId) {
        const session = agentSessionManager.getSession(conversationId);
        if (session) {
          agentSessionManager.incrementTurn(conversationId);
          agentSessionManager.updateStatus(conversationId, 'completed');
          agentSessionManager.checkpointSession(conversationId);
        }
      }
    } catch (error) {
      ...
      // Session persistence: mark failed
      if (conversationId) {
        agentSessionManager.updateStatus(conversationId, 'failed');
        agentSessionManager.checkpointSession(conversationId);
      }
    } finally {
...
// DEF-010: IPC - 从checkpoint恢复中断会话
export function registerResumeSessionIpc(): void {
  ipcMain.handle('agent:resume-session', async (_event: IpcMainInvokeEvent, { sessionId }: { sessionId: string }) => {
    try {
      const { agentSessionManager } = await import('../services/agent-session-manager.js');
      const result = await agentSessionManager.resumeFromCheckpoint(sessionId);
      if (result) {
