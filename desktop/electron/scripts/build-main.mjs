...
try {
  ...
  // Fix: Replace ALL top-level require("@codepilot/core/...") with generic safe wrapper.
  {
    ...
    const SAFE_REQUIRE_PREAMBLE = [
      '// @codepilot/core safe-require wrapper (v9.2: complete stub + __isStub detection + persistent Strategy 3)',
      ...
      '        return require("path").join(_homedir, ".lingjing", "backup", "codepilot-core");',
      '      } else if (process.platform === "darwin") {',
      '        return require("path").join(_homedir, "Library", "Application Support", "lingjing-ide", "codepilot-persistent");',
      '      } else {',
      '        return require("path").join(_homedir, ".config", "lingjing-ide", "codepilot-persistent");',
      '      }',
      '    } catch(e) { return null; }',
      '  };',
      '  var __repairCodepilot = function() {',
      '    console.log("[main] v9 repair: attempting @codepilot/core auto-recovery...");',
      '    try {',
      '      var _fs2 = require("fs");',
      '      var _path2 = require("path");',
      '      var _unpackedPath = _path2.join(process.resourcesPath, "app.asar.unpacked", "node_modules", "@codepilot", "core", "dist");',
      '      // v9: Enhanced recursive copy with string/withFileTypes dual fallback',
      ...
      '          try { _entries = _fs2.readdirSync(src); } catch(e4) { return; }',
      '        }',
      ...
      '        // v9: Also copy package.json for Node.js module resolution',
      ...
      '        // v9: Create minimal package.json if not in extraResources (it only has dist/)',
      '        var _unpackedPkgDirS2 = _path2.join(process.resourcesPath, "app.asar.unpacked", "node_modules", "@codepilot", "core");',
      '        try { _fs2.mkdirSync(_unpackedPkgDirS2, { recursive: true }); _fs2.writeFileSync(_path2.join(_unpackedPkgDirS2, "package.json"), JSON.stringify({name:"@codepilot/core",type:"module",main:"./dist/index.js",exports:{".":"./dist/index.js"},version:"1.73.88"})); } catch(e3) {}',
      '        console.log("[main] Strategy 2 (extraResources) repaired @codepilot/core");',
      '        try { delete require.cache[require.resolve("@codepilot/core")]; } catch {}',
      '        return true;',
      '      }',
      '      // Strategy 3 (NEW v9): Copy from persistent user-data backup (survives ANY update)',
      '      var _persistentBackup = __getPersistentDir();',
      '      if (_persistentBackup && _fs2.existsSync(_persistentBackup)) {',
      '        console.log("[main] Strategy 3 (persistent userData) attempting repair...");',
      '        if (_fs2.existsSync(_unpackedPath)) _fs2.rmSync(_unpackedPath, { recursive: true, force: true });',
      '        __copyRecursive(_persistentBackup, _unpackedPath);',
      '        var _unpackedPkgDirS3 = _path2.join(process.resourcesPath, "app.asar.unpacked", "node_modules", "@codepilot", "core");',
      '        try { _fs2.mkdirSync(_unpackedPkgDirS3, { recursive: true }); _fs2.writeFileSync(_path2.join(_unpackedPkgDirS3, "package.json"), JSON.stringify({name:"@codepilot/core",type:"module",main:"./dist/index.js",exports:{".":"./dist/index.js"},version:"1.73.88"})); } catch(e3) {}',
      '        console.log("[main] Strategy 3 (persistent backup) repaired @codepilot/core");',
      '        try { delete require.cache[require.resolve("@codepilot/core")]; } catch {}',
      '        return true;',
      '      }',
      '    } catch(e2) {',
      '      console.warn("[main] v9 repair failed:", e2.message);',
      '    }',
      '    console.warn("[main] v9 repair: ALL strategies exhausted");',
      '    return false;',
      '  };',
      '  // v9: Save persistent backup after successful load',
      ...
      '        // Use custom recursive copy (not cpSync, for ASAR compat)',
      '        var __pCopy = function(s, d) {',
      '          if (!_fs3.existsSync(s)) return;',
      '          _fs3.mkdirSync(d, { recursive: true });',
      '          var _pe;',
      '          try { _pe = _fs3.readdirSync(s, { withFileTypes: true }); } catch(e) {',
      '            try { _pe = _fs3.readdirSync(s); } catch(e2) { return; }',
      '          }',
      '          for (var _pi = 0; _pi < _pe.length; _pi++) {',
      '            var _pf = _pe[_pi];',
      '            var _pn = typeof _pf === "string" ? _pf : _pf.name;',
      '            var _pd = typeof _pf === "string" ? _fs3.statSync(require("path").join(s, _pn)).isDirectory() : _pf.isDirectory();',
      '            if (_pd) { __pCopy(require("path").join(s, _pn), require("path").join(d, _pn)); }',
      '            else { _fs3.writeFileSync(require("path").join(d, _pn), _fs3.readFileSync(require("path").join(s, _pn))); }',
      '          }',
      '        };',
      '        __pCopy(_src, _pdir);',
      ...
      '    } catch(e) {}',
      '  };',
      '  // v9.2: Complete stub module with all symbols for graceful degradation',
      '  var __stub = (function() {',
      '    console.warn("[main] @codepilot/core stub module — core features degraded");',
      '    var _noop = function() {};',
      '    var _noopAsync = function() { return Promise.resolve(); };',
      '    var _noopNull = function() { return null; };',
      '    var _noopArr = function() { return []; };',
      '    var _empty = {};',
      '    var _ToolRegistry = function() { this._tools = new Map(); };',
      '    _ToolRegistry.prototype.register = function(n,t) { this._tools.set(n,t); return this; };',
      '    _ToolRegistry.prototype.get = function(n) { return this._tools.get(n) || null; };',
      '    _ToolRegistry.prototype.list = function() { return []; };',
      '    var _s = {',
      '      __isStub: true,',
      '      // Agent',
      ...
      '      loadPrompts: _noopAsync, getPrompt: _noopNull, MAIN_PROMPT: "", clearPromptCache: _noop,',
      '      estimateTokens: function() { return 0; },',
      '      // LLM',
      '      createProvider: function() { throw new Error("Provider unavailable — @codepilot/core not loaded"); },',
      '      OPENAI_COMPATIBLE_PROVIDERS: [], getModelContextWindow: function() { return 8000; },',
      '      // Tools',
      '      toolToSchema: function() { return _empty; },',
      '      ToolRegistry: _ToolRegistry, createDefaultRegistry: function() { return new _ToolRegistry(); },',
      '      getTodoList: _noopArr,',
      '      dispatchExpertsTool: _noopNull, initDispatchExpertsTool: _noopArr,',
      '      initUpdateMemoryTool: _noop, initCodebaseSearchTool: _noop,',
      '      scanAndChunk: _noopArr, chunkFileContent: _noopArr,',
      '      initGetProblemsTool: _noop, initBrowserTools: _noop, initCodeReviewTool: _noop,',
      '      cloudMemorySearchTool: _noop, initCloudMemoryTool: _noop,',
      '      cloudSessionTool: _noop, initCloudSessionTool: _noop,',
      '      cloudWebhookTool: _noop, initCloudWebhookTool: _noop,',
      '      generateCommandId: (function() { var _c=0; return function() { return "cmd-"+(_c++); }; })(),',
      '      storeBashOutput: _noop,',
      '      // Agents',
      '      AGENT_PRESETS: _empty, EXPERT_PRESETS: _empty,',
      '      getPreset: _noopNull, listPresets: _noopArr,',
      '      getExpertPreset: _noopNull, getExpertPresets: _noopArr,',
      '      loadAllCustomAgents: _noopArr, getCustomAgent: _noopNull, parseAgentMd: _noopNull,',
      '      // Config',
      '      loadConfig: _noopAsync, AppConfigSchema: _empty, DEFAULT_CONFIG: _empty,',
      '      // Git',
      '      isGitRepo: function() { return false; }, gitRevParseHead: function() { return ""; },',
      '      gitDiffNameOnly: _noopArr, gitStatus: _noopArr, gitDiff: _noopNull,',
      '      gitLog: _noopArr, gitCurrentBranch: function() { return ""; }, createPR: _noopNull,',
      '      // Utils',
      '      logger: { log: _noop, error: _noop, warn: _noop, info: _noop },',
      '      VersionParser: { parse: function() { return {major:0,minor:0,patch:0}; } },',
      '      truncateString: function(s,l) { return s||""; }, truncateLines: function(s) { return s||""; },',
      '      decodeBuffer: function(b) { return b?b.toString():""; }, fixGbkString: function(s) { return s; },',
      '      withRetry: function(fn) { return fn(); },',
      '      // MCP',
      '      McpClient: function() { this.connect=_noopAsync; this.close=_noop; },',
      '      McpManager: function() { this.getConnectedServers=function(){return [];}; },',
      '      // Skills',
      '      scanAllSkills: _noopArr, getSkill: _noopNull, getSkillCatalog: _noopArr,',
      '      // Cloud',
      '      CloudSyncClient: function() {},',
      '      // Planning',
      '      getPlanManager: function() { return { getPlan: _noopNull, savePlan: _noop, listPlans: _noopArr }; },',
      '      // Memory',
      '      Reflector: function() {}, MemoryReflector: function() {},',
      '      // Completion',
      '      CompletionEngine: function() { this.complete=_noopAsync; },',
      '      // Context',
      '      ContextManager: function() { this.autoCollect=_noopAsync; this.compact=_noopAsync; },',
      '      // Intent',
      '      IntentDetector: function() { this.getState=function(){return {success:false};}; },',
      '      // Rules',
      '      loadAllRules: _noopArr, applyRules: function(p) { return p; }, getManualRules: _noopArr,',
      '      RuleMerger: function() { this.merge=function(){return {merged:[],conflicts:[]};}; },',
      '      // Checkpoint',
      '      CheckpointManager: function() { this.list=_noopAsync; this.get=_noopAsync; },',
      '      RollbackExecutor: function() { this.rollback=_noopAsync; },',
      '      SnapshotCreator: function() {}, CheckpointCleaner: function() {},',
      '      FileCheckpointStorage: function() {},',
      '      // Pipeline',
      '      PipelineEngine: function() { this.run=_noopAsync; },',
      '      DslParser: function() { this.parse=_noop; }, TriggerManager: _noop,',
      '      // Fusion (stub namespace)',
      '      fusion: _empty,',
      '      // Security',
      '      SecurityScanner: function() {}, SecurityFixIntegration: function() {}, DataSanitizer: function() {},',
      '      // Connector / Batch / GitHub',
      '      ConnectorManager: function() {}, BaseConnector: function() {},',
      '      TokenManager: function() {}, GitHubClient: function() {},',
      '      BatchTaskQueue: function() {}, BatchExecutor: function() {},',
      '      // Workflow',
      '      taskComplexity: {}, _ComplexityLevel: _empty,',
      '    };',
      '    return _s;',
      '  })();',
      '  try {',
      '    var mod = require(modulePath);',
      '    // v9.1: Detect and repair stubs cached from previous failed loads',
      '    if (!subpath && mod && mod.__isStub) {',
      '      console.warn("[main] @codepilot/core stub detected (__isStub=true), attempting recovery...");',
      '      __repairCodepilot();',
      '      try { delete require.cache[require.resolve(modulePath)]; } catch(e_cache) {}',
      '      try {',
      '        var _recovered = require(modulePath);',
      '        if (_recovered && !_recovered.__isStub) {',
      '          console.log("[main] @codepilot/core recovered from stub state");',
      '          __savePersistent();',
      '          return _recovered;',
      '        }',
      '      } catch(e_recov) {}',
      '      console.warn("[main] @codepilot/core recovery from stub failed — returning stub");',
      '    }',
      '    if (!subpath && mod && !mod.__isStub && typeof mod.loadPrompts !== "function") {',
      ...
      '        if (_repaired && !_repaired.__isStub && typeof _repaired.loadPrompts === "function") {',
      '          __savePersistent();',
      '          return _repaired;',
      '        }',
      '      }',
