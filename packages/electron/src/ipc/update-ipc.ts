  ...
  ipcMain.handle('update:download', async () => {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        // IMPORTANT: Do NOT call checkForUpdates() here — it emits 'update-available'
        // which resets the renderer state from 'downloading' back to 'available',
        // causing the progress bar to disappear. The update info is already cached
        // from the initial check that showed the "download" button.
