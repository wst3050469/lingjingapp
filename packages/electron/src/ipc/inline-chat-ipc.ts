...
let requestSeq = 0; // Monotonic sequence to detect stale/overlapping requests
...
export function registerInlineChatIpc(): void {
  ipcMain.handle('inline-chat:generate', async (_event, params: InlineChatRequest) => {
    // Increment request sequence to track overlapping requests
    ...
    try {
      ...
      for await (const event of provider.chat(request)) {
        // Check both signal and sequence staleness
        if (signal.aborted) break;
        if (seq !== requestSeq) {
          // Newer request arrived — this one is stale, abort
          ac.abort();
          break;
        }
      }

      // DEF-014: If this request was superseded by a newer one, return explicit cancellation
      if (seq !== requestSeq) {
        return { code: '', error: 'Request superseded by newer inline-chat request' };
      }
      ...
      if (err?.name === 'AbortError' || signal.aborted) {
        // DEF-014: Distinguish user-initiated abort from superseded request
        if (seq !== requestSeq) {
          return { code: '', error: 'Request superseded' };
        }
