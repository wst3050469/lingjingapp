...
describe('SlidingWindowMemoryManager', () => {
  let mgr: SlidingWindowMemoryManager;
  let eventBus: IEventBus;

  beforeEach(() => {
    eventBus = { publish: vi.fn(), subscribe: vi.fn(), unsubscribe: vi.fn() } as unknown as IEventBus;
    mgr = new SlidingWindowMemoryManager();
    mgr.initialize(eventBus, {} as IHookRegistry);
  });

  function makeMsg(content: string, role: string = 'user', idx: number = 0): Message {
