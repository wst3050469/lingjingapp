...
/** Iframe load timeout (ms) — if onLoad doesn't fire within this time, show error */
    ...
    loadTimerRef.current = setTimeout(() => {
      setIsLoading((loading) => {
        if (loading && currentUrl) {
          setLoadError(`无法连接到 ${currentUrl}，请确认开发服务器已启动。`);
        }
        return false;
      });
    }, IFRAME_LOAD_TIMEOUT);
      ...
      <div className="flex-1 relative bg-white">
        {!loadedUrl ? (
          ...
          <>
            ...
            {loadError && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-cp-bg z-20">
                <svg className="w-8 h-8 text-red-400/60 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                </svg>
                <div className="text-red-400/80 text-[11px] mb-2 px-6 text-center">
                  {loadError}
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <button
                    onClick={handleReload}
                    className="text-[10px] px-2.5 py-1 rounded-md bg-white/10 text-white/80 hover:bg-white/15 transition-colors"
                  >
                    重试
                  </button>
                  <button
                    onClick={() => setLoadError(null)}
                    className="text-[10px] px-2.5 py-1 rounded-md text-white/60 hover:text-white/80 transition-colors"
                  >
                    关闭
                  </button>
                </div>
