...
import { scanAllSkills, getSkill, getSkillCatalog, type SkillConfig } from '@codepilot/core';
...
export function registerSkillsIpc(getWorkspace: () => string): void {
  ...
  ipcMain.handle('skills:list', async () => {
    try {
      ...
      // @ts-ignore -- scanAllSkills return type not fully resolved
      return (Array.from(skillsMap.values()) as any[]).map(skill => ({
        name: skill.name,
