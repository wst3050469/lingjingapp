...
function QuestWelcome() {
  ...
  const handleSend = useCallback(async () => {
    ...
    // Quick database health check before creating task
    try {
      // @ts-ignore -- db API exists at runtime but not in ElectronAPI type
      const health = await window.electronAPI.db.health();
      if (!health?.healthy) {
        const missingInfo = health?.missingCritical?.length 
          ? `缺失关键表: ${health.missingCritical.join(', ')}。` 
          : '';
        setError(`数据库不健康，无法创建任务。${missingInfo}请删除 ${health?.dbPath || '~/.lingjing/lingjing.db'} 后重启应用。`);
        setText(message);
        return;
      }
    } catch (dbErr) {
      console.warn('[Quest] db:health check failed:', dbErr);
      // Non-blocking: proceed even if health check fails (might be IPC not registered yet)
    }
    ...
    try {
      ...
      if (!taskId) {
        setText(message);
        // Show specific error from the store for better diagnostics
        const lastError = useQuestStore.getState().lastError;
        setError(lastError
          ? `任务创建失败: ${lastError}`
          : '任务创建失败，数据库可能未初始化。请打开开发者工具 (F12) 查看控制台详情。');
        return;
      }
