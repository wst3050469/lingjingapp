...
import { executeInitScripts, loadWorktreeInitConfig } from './init-script-executor.js';
...
export function registerQuestIpc(mainWindow: BrowserWindow, getWorkspace: () => string): void {
  ...
  try {
    ...
    ipcMain.handle('quest:create-task', async (_event, params: {
      ...
      try {
        ...
        // ── Pre-check: verify quest_tasks table exists ──
        try {
          const tableCheck = db.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='quest_tasks'");
          if (!tableCheck || tableCheck.length === 0 || tableCheck[0].values.length === 0) {
            sendLog('[Quest IPC] CRITICAL: quest_tasks table does not exist in database!');
            // List all available tables for diagnostics
            const allTables = db.exec("SELECT name FROM sqlite_master WHERE type='table'");
            const tableNames = allTables.length > 0 ? allTables[0].values.map((r: any) => r[0]).join(', ') : '(none)';
            return { error: `数据库表 quest_tasks 不存在。当前表: ${tableNames}。请删除 ~/.lingjing/lingjing.db 后重启应用。` };
          }
          sendLog('[Quest IPC] quest_tasks table exists');
        } catch (checkErr) {
          sendLog('[Quest IPC] Table check failed:', checkErr);
          return { error: `数据库表检查失败: ${checkErr instanceof Error ? checkErr.message : String(checkErr)}` };
        }
        ...
        if (errMsg.includes('no such table')) {
          return { error: `数据库表缺失: ${errMsg}。请删除 ~/.lingjing/lingjing.db 后重启应用重建数据库。` };
        }
        if (errMsg.includes('NOT NULL constraint failed')) {
          return { error: `数据库约束失败: ${errMsg}。请删除 ~/.lingjing/lingjing.db 后重启应用重建数据库。` };
        }
        if (errMsg.includes('Database not initialized')) {
          return { error: `数据库未初始化。请关闭应用并重新打开。如果问题持续，请删除 ~/.lingjing/lingjing.db。` };
        }
        if (errMsg.includes('disk I/O error') || errMsg.includes('ENOSPC') || errMsg.includes('permission')) {
          return { error: `磁盘写入失败: ${errMsg}。请检查磁盘空间和权限。` };
        }
        return { error: errMsg };
      }
  ...
  ipcMain.handle('quest:delete-task', async (_event, { taskId }: { taskId: string }) => {
    ...
    if (taskAgent) {
      ...
      // Sync pause with unified SessionManager
      agentSessionManager.pauseSession(taskId);

      // Sync with unified SessionManager
      agentSessionManager.destroySession(taskId);
  ...
  ipcMain.handle('quest:run', async (_event, {
    ...
    if (runMode === 'worktree') {
      ...
      if (isRepo) {
        ...
        try {
          ...
          // GAP-005: Auto-run init scripts after worktree creation
          try {
            const initConfig = loadWorktreeInitConfig();
            if (initConfig.npmInstall || initConfig.commands.length > 0) {
              sendQuestEvent({ type: 'init_start', taskId, message: 'Running init scripts...' });
              executeInitScripts(worktreePath, initConfig, (log) => {
                sendQuestEvent({ type: 'init_log', log, taskId });
              }).then((initResult) => {
                if (initResult.success) {
                  sendQuestEvent({ type: 'init_complete', taskId });
                } else {
                  sendQuestEvent({ type: 'init_error', taskId, error: { message: `Init failed at: ${initResult.failedStep}` } });
                }
              });
            }
          } catch (err) {
            sendQuestEvent({ type: 'init_error', taskId, error: { message: `Init error: ${err instanceof Error ? err.message : String(err)}` } });
          }
    ...
    // Register with unified SessionManager for persistence & recovery
    agentSessionManager.registerSession({
      sessionId: taskId,
      type: 'quest',
      taskTitle: message.slice(0, 80),
      agent,
      abortController,
    }).catch(() => {});
  ...
  ipcMain.handle('quest:abort', async (_event, { taskId }: { taskId: string }) => {
    ...
    if (taskAgent) {
      ...
      // Sync with unified SessionManager
      agentSessionManager.updateStatus(taskId, 'cancelled');
      agentSessionManager.checkpointSession(taskId);
  ...
  ipcMain.handle('quest:resume', async (_event, { taskId, message, runId }: { taskId: string; message?: string; runId?: string }) => {
    ...
    // Restore from checkpoint or register with SessionManager
    agentSessionManager.restoreSession(taskId).then((restored) => {
      if (!restored) {
        agentSessionManager.registerSession({
          sessionId: taskId,
          type: 'quest',
      ...
      agentSessionManager.updateStatus(taskId, 'running');
    }).catch(() => {});
