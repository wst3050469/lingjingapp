...
import type { IChildProcess, IChildProcessHandle } from '../process-manager.js';
...
describe('OpenSpace process-manager', () => {
  ...
  describe('state management', () => {
    ...
    it('should skip duplicate state changes', () => {
      ...
      (pm as any).setState('stopped'); // already stopped
  ...
  describe('healthCheck', () => {
    ...
    it('should report healthy with process handle and ws port', () => {
      const mockProcess: IChildProcessHandle = {
        pid: 12345,
        stdout: { on: vi.fn() },
        stderr: { on: vi.fn() },
        on: vi.fn(),
      ...
      const pm = new OpenSpaceProcessManager();
      (pm as any).processHandle = mockProcess;
      pm.setWebSocketPort(4680);
  ...
  describe('installation detection', () => {
    function createMockProcessApi(existingCommands: Record<string, { stdout: string; stderr?: string; fail?: boolean }>): IChildProcess {
      return {
        spawn: vi.fn() as any,
  ...
  describe('start/stop', () => {
    function createMockProcessHandle(overrides: Partial<IChildProcessHandle> = {}): IChildProcessHandle {
      return {
        pid: 99999,
        stdout: { on: vi.fn() },
        stderr: { on: vi.fn() },
    ...
    function createMockApi(handle?: IChildProcessHandle): IChildProcess {
      const mockExecSync = vi.fn((cmd: string) => {
        if (cmd.includes('--version')) return 'OpenSpace v0.19.0\n';
        if (cmd.includes('which') || cmd.includes('test')) return '/usr/local/bin/openspace';
        return '';
      });
      return {
    ...
    it('should start successfully with valid installation', async () => {
      ...
      expect(mockApi.spawn).toHaveBeenCalledWith(
        '/usr/local/bin/openspace',
        ['--profile', 'default', '--windowless'],
        expect.any(Object),
      );
    });
