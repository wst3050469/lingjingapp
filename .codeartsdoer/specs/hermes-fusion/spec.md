...
2. 灵境系统现有架构信息（现有架构：Core/Electron/Renderer三层 + fusion融合层；规划架构：详见本文档融合设计、Skills系统、内存系统、工具系统等）
3. 灵境系统现有能力基线（已实现特性的代码位置与接口）
4. 融合约束条件（非侵入式原则、插件化要求、兼容性保障）

### 1.3 核心输出

1. 功能对比矩阵（Hermes特性 vs 灵境现状 vs 融合策略，交叉引用：详见full-completion/spec.md的REQ-FC系列需求）
...
- **LLM服务（含国内模型：百度文心、腾讯混元、Kimi、通义千问、豆包、GLM、MiniMax等）**：提供大语言模型推理能力的外部服务
...
rectangle "外部服务" {
    usecase "LLM服务（含国内模型：百度文心、腾讯混元、Kimi、通义千问、豆包、GLM、MiniMax等）" as LLM
...
|------|-----------|---------|---------|---------|
| H01 | 滑动窗口短期记忆 | Agent循环中auto-compaction已部分实现 | auto-compaction是压缩策略（注：仅压缩历史层，工作层快速增长时可能无法降回阈值），非滑动窗口淘汰；需补充窗口滑动逻辑 | **增强**：在auto-compaction前增加滑动窗口淘汰阶段，作为Hook注入Agent循环 |
| H02 | 向量长期记忆 | update_memory工具+SQLite存储（category+scope） | 现有记忆为结构化KV存储，无语义向量检索能力 | **现有实现**：向量存储适配器已存在于fusion/vector-memory/中，作为独立旁路模块接入，与SQLite记忆并行存在 |
| H03 | Nudge后台审查引擎 | MemoryNudger（每3轮提醒持久化，注：提醒消息仅注入对话，无转发至跨会话记忆系统的逻辑） | 现有Nudger仅做提醒，非质量审查；ReviewAgent已实现但Nudger→ReviewAgent融合待实施 | **现有实现**：ReviewAgent实例已存在于fusion/review-engine/中（nudge-review-engine.ts），通过事件总线订阅主Agent输出，异步审查；**待实施**：MemoryNudger与ReviewAgent融合 |
...
| H06 | DAG任务编排 | WorkflowExecutor（流程执行引擎） | 现有WorkflowExecutor能力未明确是否支持DAG | **现有实现**：DAG编排模块已存在于fusion/dag-orchestrator/中（dag-orchestrator.ts），通过适配器对接WorkflowExecutor |
| H07 | 多Agent并行执行器 | 单Agent循环 | 无多Agent并行能力 | **现有实现**：ParallelExecutor模块已存在于fusion/multi-agent/中（multi-agent-executor.ts），管理多个Agent实例生命周期，通过事件总线汇聚结果 |
| H08 | 动态模型路由 | LLM抽象层（config/schema.ts + config/defaults.ts（注：src/config/目录不存在，仅有dist/config/编译产物）） | 现有模型选择为静态配置，无动态路由 | **现有实现**：ModelRouter模块已存在于fusion/model-router/中（dynamic-model-router.ts），通过Hook拦截LLM调用，根据任务特征动态选择模型 |
| H09 | 自然语言Cron调度 | Cloud Server已有Scheduler | 现有Scheduler需手动配置Cron表达式，无自然语言转换 | **增强**：在Scheduler前增加NL-to-Cron转换层，作为独立适配器模块 |
| H10 | Honcho用户建模 | MemoryReflector（周期性聚合提炼用户画像和项目知识，创建时需传入provider参数） | MemoryReflector已部分实现用户画像，但未与Agent行为深度联动 | **增强**：扩展MemoryReflector输出，通过事件总线发布用户模型更新事件，供其他模块订阅 |
...
**功能行为**：
- 当对话上下文token数超过配置的窗口上限时，滑动窗口管理器应从最早的消息开始淘汰，直到token数降至窗口下限。
- 当滑动窗口淘汰执行中时，滑动窗口管理器应保留最近N条消息（N可配置，默认10）不被淘汰。
- 当滑动窗口淘汰完成时，滑动窗口管理器应发布 `memory:window_compacted` 事件，携带淘汰的消息摘要。
- 若滑动窗口管理器初始化失败或运行异常，系统应降级到原有auto-compaction策略继续运行。
...
**功能行为**：
- 当用户通过 `remember_vector` 工具存储信息时，向量长期记忆模块应将信息编码为向量表示并存储到向量数据库。
- 当用户通过 `recall_vector` 工具检索信息时，向量长期记忆模块应对查询进行向量编码，执行相似度搜索，返回Top-K结果（K可配置，默认5）。
- 当向量长期记忆模块启用时，模块应订阅 `memory:updated` 事件，自动将新写入的结构化记忆同步到向量索引。
- 若向量数据库服务不可用，向量长期记忆模块应返回降级提示，不影响其他记忆操作。
- 若向量长期记忆模块被禁用，系统行为应与融合前完全一致。
...
**功能行为**：
- 当主Agent完成一轮回复时，Nudge审查引擎应订阅 `agent:message_end` 事件，异步启动审查流程。
- 当审查流程执行中时，Nudge审查引擎应使用独立审查Agent对主Agent输出进行质量评估，生成审查报告。
- 当审查报告生成完成时，Nudge审查引擎应发布 `review:completed` 事件，携带审查建议和评分。
- Nudge审查引擎应将审查建议标记为"审查建议"，不得自动修改主Agent输出。
- 若审查模型调用失败或超时，Nudge审查引擎应发布 `review:failed` 事件并静默降级，不干扰主流程。
- Nudge审查引擎应最多占用1个LLM并发配额。
...
**功能行为**：
- 当Agent执行工具调用时，执行轨迹收集器应订阅 `agent:tool_call` 和 `agent:tool_result` 事件，记录完整调用序列。
- 当一轮对话结束且执行轨迹包含3个以上工具调用时，轨迹技能生成器应分析执行轨迹，提取可复用工作流模式。
- 当工作流模式提取成功时，轨迹技能生成器应生成SKILL.md文件（标记为auto-generated级别），通过SkillAdapter注册到Skills系统。
- 若轨迹分析未能提取有效模式，轨迹技能生成器应静默跳过，不生成无效Skill。
- 若轨迹技能生成功能被禁用，SkillHarvester应仅使用原有对话分析模式生成Skill。
...
**功能行为**：
- 当Skill被请求加载时，安全扫描器应在 `before_skill_load` Hook点执行安全审查，检测命令注入、路径遍历、权限提升、敏感数据泄露。
- 若安全扫描发现高风险问题，安全扫描器应阻止该Skill加载，发布 `skill:blocked` 事件，携带风险详情。
- 若安全扫描发现中低风险问题，安全扫描器应为Skill附加安全修补建议，允许加载但标记为"受限"。
- 当Skill通过安全扫描时，渐进加载器应仅加载Skill的元数据和入口描述，不加载完整内容。
- 当Skill被实际调用执行时，渐进加载器应按需加载Skill的完整定义和依赖。
- 若安全扫描功能被禁用，Skills系统应使用原有加载流程。
...
**功能行为**：
- 当用户提交DAG编排请求时，DAG编排引擎应解析DAG定义，验证无环路，构建执行计划。
- 当DAG执行中时，DAG编排引擎应按照依赖关系调度子任务，无依赖的子任务并行执行，有依赖的串行执行。
- 当DAG子任务包含条件分支时，DAG编排引擎应根据上游任务输出评估条件，选择执行分支。
- 当DAG所有子任务执行完成时，DAG编排引擎应汇聚结果，发布 `dag:completed` 事件。
- 若DAG执行中某子任务失败，DAG编排引擎应标记该节点及下游节点为失败，发布 `dag:failed` 事件，支持从失败节点重试。
- 若DAG定义存在环路，DAG编排引擎应拒绝执行并返回验证错误。
...
**功能行为**：
- 当用户提交并行执行请求（包含多个独立子任务）时，并行执行器应为每个子任务创建独立Agent实例。
- 当并行执行中时，并行执行器应同时运行所有Agent实例，并发数不超过配置上限（默认4）。
- 当所有Agent实例完成执行时，并行执行器应汇聚各子任务结果，发布 `parallel:completed` 事件。
- 若某Agent实例执行超时，并行执行器应终止该实例，将其结果标记为超时，继续等待其他实例完成。
- 若并发配额不足，并行执行器应降级为串行执行，按顺序依次处理子任务。
- 并行执行器应在执行完成后释放所有Agent实例资源。
...
**功能行为**：
- 当主Agent发起LLM调用时，动态模型路由器应在 `before_llm_call` Hook点拦截请求，评估任务特征（类型、复杂度、上下文长度）。
- 当评估任务特征时，动态模型路由器应根据路由规则选择最优模型（考虑能力匹配、成本预算、当前负载）。
- 当模型选择完成时，动态模型路由器应替换请求中的模型标识为选定模型，记录路由决策日志。
- 若无匹配路由规则，动态模型路由器应使用默认模型配置，不修改请求。
- 若选定模型不可用，动态模型路由器应自动降级到备选模型，发布 `model:fallback` 事件。
- 若动态路由功能被禁用，系统行为应与融合前完全一致（使用静态配置模型）。
...
**功能行为**：
- 当用户以自然语言描述定时任务需求（如"每天早上9点执行代码审查"）时，NL调度器应将自然语言转换为标准Cron表达式。
- 当Cron表达式生成完成时，NL调度器应通过SchedulerAdapter将任务注册到Cloud Server Scheduler。
- 当任务注册成功时，NL调度器应返回任务ID和Cron表达式给用户确认。
- 若自然语言无法转换为有效Cron表达式，NL调度器应返回解析错误并提供示例格式提示。
- 若NL调度功能被禁用，Scheduler应仅接受标准Cron表达式输入。
...
**功能行为**：
- 当MemoryReflector完成一轮用户画像更新时，用户建模增强模块应发布 `user_model:updated` 事件，携带用户偏好模型数据。
- 当其他增强模块订阅 `user_model:updated` 事件时，订阅模块应根据用户偏好调整自身行为（如模型路由偏好、记忆检索权重等）。
- 当用户模型持续更新时，用户建模增强模块应合并增量更新到完整用户模型，避免模型丢失历史信息。
- 若用户模型发布事件失败，用户建模增强模块应静默降级，不影响MemoryReflector原有功能。
...
**功能行为**：
- 当新的消息平台连接器（如Discord、Slack）被注册到ConnectorHub时，统一消息网关应通过标准适配器接口接入该平台。
- 当外部平台发送消息到灵境时，统一消息网关应将平台消息转换为统一的内部消息格式，路由到Agent处理。
- 当Agent生成回复时，统一消息网关应将内部消息格式转换回目标平台格式，发送回复。
- 若某平台连接器初始化失败，统一消息网关应标记该平台为不可用，不影响其他平台消息收发。
- 若新平台连接器未注册，系统行为应与融合前完全一致。
...
**功能行为**：
- 当融合模块需要注册新连接器时，连接器适配器应通过ConnectorHub的标准注册接口完成注册。
- 当融合模块需要发现可用连接器时，连接器适配器应通过ConnectorHub的发现接口返回连接器列表。
- 连接器适配器应不修改ConnectorHub的任何接口或实现。
...
**功能行为**：
- 当事件总线初始化时，事件总线应注册所有预定义事件主题和默认订阅者。
- 当发布者发布事件时，事件总线应将事件投递到所有匹配主题的订阅者，投递延迟不超过10ms（P99）。
- 当事件总线运行中时，事件总线应支持事件过滤（按主题、来源、优先级）和超时跳过（订阅者回调超时100ms自动跳过）。
- 若订阅者回调执行异常，事件总线应捕获异常，记录错误日志，不中断事件投递到其他订阅者。
- 事件总线应支持至少1000事件/秒的吞吐量。
...
**功能行为**：
- 当Hook注册中心初始化时，Hook机制应在预定义的执行点（before_llm_call、after_llm_call、before_tool_execute、after_tool_execute、before_skill_load、after_skill_load、before_memory_write、after_compaction）建立注册接口。
- 当融合模块注册Hook回调时，Hook机制应按优先级排序注册的回调，支持同步和异步模式。
- 当执行点到达时，Hook机制应按优先级依次调用注册的回调，异步回调不阻塞主流程。
- 若Hook回调执行超时（超过100ms），Hook机制应自动跳过该回调，继续执行后续回调。
- 若Hook回调执行异常，Hook机制应捕获异常，记录错误日志，不中断主流程。
...
| 问题编号 | 问题描述 | 修复方式 |
|---------|---------|---------|
| F-1 | nudger.ts源码路径标注错误 | 标注源码位于dist编译产物中，src/memory/下仅有reflector.ts |
| F-2 | update-memory.ts/cloud-memory.ts路径 | 标注tools/builtin/目录不存在于src中 |
| F-3 | config/schema.ts和config/defaults.ts路径 | 标注src/config/目录不存在，仅有dist/config/编译产物 |
| F-5 | Tool接口riskLevel属性 | 标注为OpenSpace安全扫描模块专有，非通用属性 |
| F-6 | StructuredError类不存在 | 标注为需要新建 |
| F-8 | Reflector创建需要provider参数 | 标注创建时需传入provider参数 |
| L-1 | Token预算三层占比矛盾 | 标注占比范围约束与求和=1.0存在矛盾 |
| L-3 | Nudger提醒消息无跨会话转发 | 标注仅注入对话，无转发至跨会话记忆系统逻辑 |
| L-4 | reflect()从未被调用 | 标注修复方案待实施 |
| L-5 | 上下文压缩仅压缩历史层 | 标注工作层快速增长时可能无法降回阈值 |
| M-1 | Fusion模块已有实现 | 将H02/H03/H06/H07/H08从新增改为现有实现 |
| M-7 | LLM Provider遗漏国内模型 | 补充百度文心、腾讯混元、Kimi、通义千问、豆包、GLM、MiniMax等 |
| C-1 | Reflector调用状态矛盾 | H03标注待实施：MemoryNudger与ReviewAgent融合 |
| C-3 | 分层架构与实际代码不符 | 标注现有架构vs规划架构 |
| C-4 | 与full-completion描述深度差异 | 建立交叉引用关系 |
