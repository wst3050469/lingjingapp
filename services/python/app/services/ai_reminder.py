...
async def _get_active_tenants() -> list[dict]:
    ...
    async with database.pool.acquire() as conn:
        try:
            ...
            logger.warning(f"查询活跃租户(含company_name)失败, 降级: {e}")
            rows = await conn.fetch(
                "SELECT tenant_id, config, plan FROM tenants WHERE status='active'"
            )
            return [
                {"tenant_id": r["tenant_id"], "company_name": r["tenant_id"],
                 "config": json.loads(r["config"]) if isinstance(r["config"], str) else (r["config"] or {}),
                 "plan": r["plan"]}
                for r in rows
            ]
