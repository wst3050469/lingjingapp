 ...
 * BUG-022: Performs a dry-run merge first to detect conflicts before committing.
 */
export async function mergeWorktreeChanges(
  ...
  try {
    ...
    // BUG-022: Step 1 — Try merge without committing to detect conflicts
    try {
      await execFileAsync(
        'git',
        ['merge', branchName, '--no-commit', '--no-ff'],
        { cwd: repoRoot, timeout: 15000 }
      );
    } catch (mergeErr: any) {
      ...
      try {
        await execFileAsync('git', ['merge', '--abort'], { cwd: repoRoot, timeout: 5000 });
      } catch { /* ignore */ }

      if (conflictedFiles.length > 0) {
        return {
          success: false,
          error: `${conflictedFiles.length} file(s) have merge conflicts`,
          conflictedFiles,
        };
      }
      return {
    ...
    // BUG-022: Step 2 — No conflicts, proceed with commit
    try {
      await execFileAsync(
        'git',
        ['commit', '-m', commitMessage, '--no-edit'],
        { cwd: repoRoot, timeout: 10000 }
      );
    } catch (commitErr) {
      // If commit fails (maybe nothing to commit), abort the merge
      try {
        await execFileAsync('git', ['merge', '--abort'], { cwd: repoRoot, timeout: 5000 });
      } catch { /* ignore */ }
      return {
    ...
    // Top-level catch — abort any in-progress merge
    try {
      await execFileAsync('git', ['merge', '--abort'], { cwd: repoRoot, timeout: 5000 });
    } catch { /* ignore */ }
    return {
