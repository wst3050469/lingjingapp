...
- **LLM 服务**：提供大语言模型推理能力，用于自然语言到脚本转换
...
1. **安装路径检测**：灵境必须在首次使用前检测 OpenSpace 安装路径，支持自动搜索（PATH 环境变量、常见安装目录）和手动指定
   ...
   a. 验收条件：[用户首次打开 OpenSpace 功能面板] → [系统自动搜索 OpenSpace 可执行文件，若未找到则提示用户手动指定路径]

2. **进程启动**：灵境应能以子进程方式启动 OpenSpace，并传递必要的启动参数（Profile 路径、窗口模式、同步配置）

   a. 验收条件：[用户点击"启动 OpenSpace"] → [系统以子进程方式启动 OpenSpace 并建立 WebSocket 通信通道，状态变为"运行中"]

3. **进程停止**：灵境应能优雅停止 OpenSpace 进程（先发送退出命令，超时后强制终止）

   a. 验收条件：[用户点击"停止 OpenSpace"] → [系统向 OpenSpace 发送退出命令，若 10s 内未退出则强制终止进程，状态变为"已停止"]

4. **健康检查**：灵境应周期性检查 OpenSpace 进程存活状态和通信可用性

   a. 验收条件：[OpenSpace 进程运行中且通信正常] → [健康状态为"健康"，每 5s 检查一次]
   b. 验收条件：[OpenSpace 进程异常退出或通信中断] → [健康状态变为"异常"，10s 内通知用户并尝试自动重连]

5. **版本兼容性检测**：启动时应检测 OpenSpace 版本号并与兼容版本范围比较

   a. 验收条件：[检测到的 OpenSpace 版本在兼容范围内] → [允许启动并显示版本信息]
   b. 验收条件：[检测到的 OpenSpace 版本低于最低兼容版本] → [警告用户版本不兼容，但仍允许尝试启动]

6. **禁止项**：灵境禁止直接修改 OpenSpace 的二进制文件或配置模板

   a. 验收条件：[任何时刻] → [灵境不写入 OpenSpace 安装目录下的任何文件，Profile 等配置文件应存储在灵境工作目录中]
...
1. **通信协议选择**：灵境应支持 WebSocket（优先）和 stdio 两种通信方式与 OpenSpace 交互
   ...
   a. 验收条件：[OpenSpace 以窗口模式启动] → [使用 WebSocket 通信（端口可配置）]
   b. 验收条件：[OpenSpace 以无头模式启动] → [使用 stdio 通信]

2. **命令发送**：灵境应能向 OpenSpace 发送脚本命令并接收执行结果

   a. 验收条件：[灵境发送 Lua 脚本 `openspace.setPropertyValue("RenderEngine.Camera.FocalDistance", 100)` ] → [OpenSpace 执行脚本并返回成功/失败结果]

3. **场景状态订阅**：灵境应能订阅 OpenSpace 的场景属性变更通知

   a. 验收条件：[灵境订阅相机位置属性] → [当用户在 OpenSpace 中导航时，灵境实时接收到相机位置变更事件]

4. **命令队列管理**：当 OpenSpace 忙碌时，灵境应将命令排入队列顺序执行

   a. 验收条件：[连续发送 10 个脚本命令] → [命令按发送顺序排队执行，每个命令返回后发送下一个]

5. **双向事件桥接**：OpenSpace 事件应通过灵境 EventBus 广播，灵境事件也应在适当时机同步到 OpenSpace

   a. 验收条件：[OpenSpace 触发"场景加载完成"事件] → [灵境 EventBus 发布 `openspace:scene-loaded` 事件]
...
1. **自然语言转脚本**：灵境 AI Agent 应将用户自然语言太空指令转换为可执行的 OpenSpace 脚本
   ...
   a. 验收条件：[用户输入"飞到火星表面"] → [AI 生成 Lua 脚本 `openspace.setPropertyValue("NavigationHandler.Target", "Mars")` 等导航命令序列]

2. **多语言脚本支持**：AI 生成器应支持生成 Lua、JavaScript、Python 三种脚本语言

   a. 验收条件：[用户指定使用 JavaScript 脚本] → [AI 生成的脚本使用 JavaScript 语法]

3. **脚本上下文感知**：AI 生成脚本时应考虑当前场景状态（已加载的模块、相机位置、可见天体等）

   a. 验收条件：[当前场景已加载火星高分辨率图像] → [AI 生成"放大火星"脚本时使用火星相关属性而非通用缩放]

4. **脚本安全审查**：AI 生成的脚本在发送前应通过安全审查

   a. 验收条件：[AI 生成的脚本包含文件系统删除操作] → [安全审查拦截并提示"脚本包含危险操作，已拦截"]

5. **脚本模板库**：系统应维护常用操作的脚本模板，AI 生成脚本时优先匹配模板

   a. 验收条件：[用户输入"显示银河系"] → [AI 匹配预置模板"加载银河系数据集"而非从零生成]

6. **脚本预览与确认**：对于高风险操作（导航到新目标、加载大型数据集），应先展示脚本预览

   a. 验收条件：[AI 生成导航到新行星的脚本] → [在执行前向用户展示脚本内容，需用户确认后执行]
...
1. **Profile 文件管理**：灵境应能创建、编辑、删除、导入、导出 OpenSpace Profile 配置文件
   ...
   a. 验收条件：[用户在灵境中创建新 Profile] → [生成标准 OpenSpace Profile Lua 文件并存储在灵境工作目录]

2. **场景模块选择**：灵境应提供可视化界面让用户选择要加载的 OpenSpace 模块（太阳系、银河系、数字宇宙目录等）

   a. 验收条件：[用户勾选"太阳系"和"数字宇宙目录"模块] → [Profile 中包含对应模块的引用和配置]

3. **数据集路径管理**：灵境应能配置 OpenSpace 数据集的存储路径和缓存策略

   a. 验收条件：[用户设置数据集目录为 D:\openspace-data] → [Profile 中数据集路径引用解析到该目录]

4. **Profile 模板**：灵境应提供预设 Profile 模板（太阳系探索、深空观测、太空任务追踪等）

   a. 验收条件：[用户选择"太阳系探索"模板] → [创建包含太阳系行星、高分辨率图像、轨道线的 Profile]

5. **配置热更新**：修改 Profile 后应能热应用到运行中的 OpenSpace，无需重启进程

   a. 验收条件：[用户在运行中修改相机初始位置] → [通过脚本命令更新 OpenSpace 相机属性，无需重启]
...
1. **窗口嵌入模式**：灵境应能将 OpenSpace 渲染窗口嵌入到灵境 UI 中，支持以下模式：
   ...
   a. 验收条件：[用户选择"嵌入模式"] → [OpenSpace 渲染窗口作为灵境 UI 的子窗口显示，可拖拽调整大小]

2. **独立窗口模式**：灵境应支持 OpenSpace 以独立窗口运行

   a. 验收条件：[用户选择"独立窗口模式"] → [OpenSpace 以独立系统窗口运行，灵境通过脚本命令控制]

3. **全屏模式**：灵境应能将 OpenSpace 切换到全屏渲染模式

   a. 验收条件：[用户点击"全屏"] → [OpenSpace 渲染窗口全屏显示，按 Esc 退出全屏]

4. **多显示器支持**：灵境应支持将 OpenSpace 渲染窗口指定到特定显示器

   a. 验收条件：[用户选择显示器 2] → [OpenSpace 渲染窗口在显示器 2 上全屏显示]

5. **窗口状态同步**：灵境应感知 OpenSpace 窗口的状态变化（最大化、最小化、关闭）

   a. 验收条件：[用户在 OpenSpace 窗口点击关闭] → [灵境接收到窗口关闭事件，更新状态为"窗口已关闭"而非"进程已退出"]
...
1. **数据集目录浏览**：灵境应提供可视化界面浏览 OpenSpace 数据集目录结构
   ...
   a. 验收条件：[用户打开数据集浏览器] → [显示数据集目录树（恒星、星系、行星图像、太空任务等分类）]

2. **数据集元信息展示**：每个数据集应展示元信息（名称、大小、来源、分辨率、最后更新时间）

   a. 验收条件：[用户点击"火星高分辨率图像"数据集] → [展示元信息：名称、文件大小、分辨率、数据来源机构]>

3. **数据集状态标记**：数据集应标记其状态（已下载、未下载、部分下载、损坏）

   a. 验收条件：[数据集文件存在且校验通过] → [状态标记为"已下载"（绿色图标）]
   b. 验收条件：[数据集目录存在但文件不完整] → [状态标记为"部分下载"（黄色图标）]

4. **数据集搜索**：灵境应支持按名称、类型、标签搜索数据集

   a. 验收条件：[用户搜索"星系"] → [返回所有包含"星系"关键字的数据集列表]

5. **数据集加载控制**：用户应能从浏览器直接加载/卸载数据集到当前场景

   a. 验收条件：[用户点击"加载"按钮] → [通过脚本命令将数据集加载到 OpenSpace 当前场景]
...
1. **录制启动**：灵境应能启动 OpenSpace 的帧导出录制
   ...
   a. 验收条件：[用户点击"开始录制"并配置分辨率/帧率] → [向 OpenSpace 发送帧导出启动命令，状态变为"录制中"]

2. **录制停止**：灵境应能停止录制并获取导出帧序列信息

   a. 验收条件：[用户点击"停止录制"] → [向 OpenSpace 发送帧导出停止命令，返回帧数量和输出目录]

3. **录制配置**：灵境应提供录制参数配置界面（分辨率、帧率、输出格式、输出路径）

   a. 验收条件：[用户设置分辨率为 3840x2160、帧率为 60fps] → [录制启动时传递对应参数]

4. **回放控制**：灵境应能控制录制会话的回放（播放、暂停、跳转、速度调节）

   a. 验收条件：[用户点击"回放"并选择录制会话] → [按录制时的操作序列重放场景变化]

5. **录制会话管理**：灵境应管理录制会话列表（时间、时长、帧数、描述）

   a. 验收条件：[用户打开录制历史] → [显示所有录制会话的列表，含时间戳、时长、帧数信息]
...
1. **同步服务器连接**：灵境应能配置并连接 OpenSpace 全球同步服务器
   ...
   a. 验收条件：[用户输入同步服务器地址和端口] → [向 OpenSpace 发送同步连接命令，状态变为"已连接"]

2. **角色选择**：灵境应支持选择同步角色（主机/客机）

   a. 验收条件：[用户选择"主机"角色] → [OpenSpace 作为同步主机，其他客机跟随其场景状态]
   b. 验收条件：[用户选择"客机"角色] → [OpenSpace 跟随主机的场景状态和导航]

3. **同步状态监控**：灵境应实时显示同步连接状态（已连接/断开/延迟/客户端数量）

   a. 验收条件：[同步连接正常] → [显示"已连接"、延迟毫秒数、当前客户端数量]

4. **SyncProfile 管理**：灵境应能创建和编辑 SyncProfile 配置

   a. 验收条件：[用户创建 SyncProfile 并设置服务器地址和密码] → [生成 OpenSpace SyncProfile 文件]

5. **同步断开**：灵境应能主动断开同步连接

   a. 验收条件：[用户点击"断开同步"] → [向 OpenSpace 发送断开命令，状态变为"未连接"]
...
1. **Agent 工具注册**：灵境应在 Agent 工具系统中注册 `openspace_execute` 工具
   ...
   a. 验收条件：[灵境 Agent 工具列表] → [包含 `openspace_execute` 工具，描述为"执行 OpenSpace 脚本命令"]

2. **工具参数定义**：`openspace_execute` 工具应接受脚本内容、脚本语言类型、超时时间参数

   a. 验收条件：[Agent 调用 `openspace_execute({ script: "...", language: "lua", timeout: 10000 })` ] → [通过通信桥接层发送脚本到 OpenSpace 执行]

3. **工具返回结果**：工具应返回脚本执行结果（成功/失败、返回值、执行耗时）

   a. 验收条件：[脚本执行成功] → [返回 `{ success: true, result: "...", duration: 120 }` ]
   b. 验收条件：[脚本执行失败] → [返回 `{ success: false, error: "属性不存在", duration: 50 }` ]

4. **工具可用性守卫**：当 OpenSpace 未启动时，工具调用应返回明确错误而非静默失败

   a. 验收条件：[OpenSpace 未启动时 Agent 调用 `openspace_execute`] → [返回 `{ success: false, error: "OpenSpace 未运行，请先启动" }` ]

5. **批量脚本执行**：工具应支持一次调用执行多条脚本命令（按顺序执行）

   a. 验收条件：[Agent 调用 `openspace_execute({ scripts: ["cmd1", "cmd2", "cmd3"], language: "lua" })` ] → [按顺序执行三条命令，返回每条命令的执行结果]
...
1. **Skill 注册**：灵境应在 Skills 系统中注册 OpenSpace 相关技能，遵循三级扫描机制（builtin/user/project）
   ...
   a. 验收条件：[灵境 Skills 列表] → [包含 `openspace-navigation`、`openspace-scene-management`、`openspace-recording` 等技能]

2. **导航技能**：`openspace-navigation` Skill 应封装常见导航操作（飞到天体、设置相机、时间控制）

   a. 验收条件：[用户输入"飞到土星"] → [Skill 匹配导航意图，调用 `openspace_execute` 执行导航脚本]

3. **场景管理技能**：`openspace-scene-management` Skill 应封装场景操作（加载/卸载数据集、切换 Profile）

   a. 验收条件：[用户输入"加载火星高分辨率地图"] → [Skill 匹配场景管理意图，执行数据集加载脚本]

4. **录制技能**：`openspace-recording` Skill 应封装录制操作（开始/停止录制、回放、配置参数）

   a. 验收条件：[用户输入"录制当前演示，60帧每秒"] → [Skill 匹配录制意图，配置参数并启动录制]

5. **Skill 触发条件**：OpenSpace Skill 仅在 OpenSpace 进程运行时激活

   a. 验收条件：[OpenSpace 未运行] → [OpenSpace 相关 Skill 标记为"不可用"，不参与意图匹配]
   b. 验收条件：[OpenSpace 启动成功] → [OpenSpace 相关 Skill 标记为"可用"，参与意图匹配]

6. **Skill 自定义扩展**：用户应能在 project 级别创建自定义 OpenSpace Skill

   a. 验收条件：[用户在项目 `.skills/` 目录创建 `openspace-custom.md`] → [Skills 系统扫描并注册该自定义 Skill]
