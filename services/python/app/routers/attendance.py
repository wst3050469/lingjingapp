...
@router.get("/my-project/{user_id}")
async def get_my_project(user_id: str):
    """获取工人绑定的项目信息"""
    import db as database, json

    async with database.pool.acquire() as conn:
        # 从 tenant_users 获取 ext_data
        tu = await conn.fetchrow(
            "SELECT tenant_id, role, ext_data FROM tenant_users WHERE user_id=$1", user_id
        )
        if not tu:
            raise HTTPException(status_code=404, detail="用户不存在")
        ...
        if not project_id:
            return {"project": None, "message": "未绑定项目"}

        # 查询项目详情
        proj = await conn.fetchrow(
            """SELECT id, name, status, progress, manager_name, customer,
                      contract_amount, start_date, deadline, location
               FROM biz_projects WHERE id=$1 AND tenant_id=$2""",
            int(project_id), tu["tenant_id"],
        )

        if not proj:
            return {"project": None, "message": "项目不存在"}
        ...
        return {
            "project": {
                ...
                "contract_amount": float(proj["contract_amount"]) if proj["contract_amount"] else 0,
                "start_date": str(proj["start_date"]) if proj["start_date"] else "",
                "deadline": str(proj["deadline"]) if proj["deadline"] else "",
