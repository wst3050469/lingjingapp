...
async def list_all_tenants(
    ...
    async with database.pool.acquire() as conn:
        if status:
            ...
            rows = await conn.fetch(
                "SELECT tenant_id, company_name, category, industry, status, plan, owner_name, owner_phone, created_at "
...
from services.quota_service import (
    get_tenant_quota, set_tenant_plan, set_quota_overrides,
    get_all_usage, DEFAULT_QUOTAS,
)


@router.get("/tenants/{tenant_id}/quota")
async def get_quota(tenant_id: str, admin: dict = Depends(get_admin_user)):
    """获取租户配额详情（含用量）"""
    require_super_admin(admin)
    quota = await get_tenant_quota(tenant_id)
    usage = await get_all_usage(tenant_id)
    return {"code": 0, "data": {"quota": quota, "usage": usage}}
...
async def update_quota(
    tenant_id: str,
    plan: str = Body(None),
    overrides: dict = Body(None),
    admin: dict = Depends(get_admin_user),
):
    """设置租户配额（套餐等级 + 可选自定义上限）"""
    require_super_admin(admin)
    if plan and plan not in DEFAULT_QUOTAS:
        raise HTTPException(status_code=400, detail=f"无效套餐: {plan}，可选: {list(DEFAULT_QUOTAS.keys())}")
    if plan:
        await set_tenant_plan(tenant_id, plan)
    if overrides:
        await set_quota_overrides(tenant_id, overrides)
    quota = await get_tenant_quota(tenant_id)
    return {"code": 0, "data": {"quota": quota}, "msg": "配额已更新"}


@router.get("/plans")
async def list_plans(admin: dict = Depends(get_admin_user)):
    """列出所有可用套餐等级及默认配额"""
    require_super_admin(admin)
    return {"code": 0, "data": DEFAULT_QUOTAS}
