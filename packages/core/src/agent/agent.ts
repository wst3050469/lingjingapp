...
import type { LLMProvider, StreamEvent } from '../fusion/adapters/types.js';
 ...
 */
function looksLikeTaskComplete(text: string): boolean {
  if (!text) return true;
  if (text.length < 3) return false; // Too short to determine (e.g. "好的", "继续", "ok")
...
// Events emitted by the agent for UI rendering
export type AgentEvent =
  | { type: 'thinking' }
  | { type: 'text'; text: string }
  | { type: 'tool_start'; name: string; args: Record<string, unknown> }
  | { type: 'tool_progress'; name: string; text: string }
  | { type: 'tool_end'; name: string; result: ToolResult }
  | { type: 'error'; error: Error }
  | { type: 'usage'; inputTokens: number; outputTokens: number }
  | { type: 'intervention_injected'; text: string }
  | { type: 'code_review_report'; report: CodeReviewReport }
  ...
  async run(userMessage: string, signal?: AbortSignal, images?: Array<{ data: string; mediaType: string }>): Promise<string> {
    ...
    if (this.workflowEngine) {
      ...
      if (complexity.isComplex && complexity.confidence >= 0.7) {
        ...
        try {
          ...
          logger.error('[Agent] Failed to create workflow:', error as Error);
          // 继续正常执行
        }
      }
    }
    ...
    const heartbeatTimeoutCheck = setInterval(() => {
      ...
      if (elapsed > HEARTBEAT_TIMEOUT_MS) {
        this.emit({ type: 'heartbeat_timeout' as any, turn, elapsedMs: elapsed });
      }
    }, 10000);
    ...
    this.on('heartbeat_ack' as any, heartbeatAckHandler);
    ...
    while (turn < this.config.maxTurns) {
      ...
      const toolChoice = hasNudgedForTools ? 'required' as const : undefined;
      ...
      try {
        ...
        const stream = (this.config.provider as any).chat({
          messages,
...
Technical details: ${err.message}`;
        ...
        if (!isAbortError) {
          ...
          this.emit({ type: 'interrupted', turn, conversationSnapshot } as any);
    ...
    return responseText_fallback(this.conversation);
      ...
      this.off('heartbeat_ack' as any, heartbeatAckHandler);
  ...
  private mergeSignals(a: AbortSignal, b: AbortSignal): AbortSignal {
    try {
      ...
      return (AbortSignal as any).any?.([a, b]) ?? this.mergeSignalsManual(a, b);
    } catch {
      return this.mergeSignalsManual(a, b);
    }
  }

  private mergeSignalsManual(a: AbortSignal, b: AbortSignal): AbortSignal {
    const controller = new AbortController();
    const onAbort = () => { controller.abort((a as any).reason ?? (b as any).reason); };
...
function responseText_fallback(conversation: Conversation): string {
