import type { ScriptTemplate, ScriptLanguage } from './types.js';
...
export function matchTemplate(input: string): ScriptTemplate | null {
  const normalized = input.toLowerCase().trim();
  const inputWords = normalized.split(/\s+/);

  let bestMatch: ScriptTemplate | null = null;
  let bestScore = 0;
...
export function fillTemplate(template: ScriptTemplate, params: Record<string, string | number>): string {
