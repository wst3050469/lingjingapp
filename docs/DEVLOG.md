...
## 2026-06-04 v1.64.19 — 支出记录+供应商 新增创建表单（全部模块CRUD完成）✅

### 完成内容
为项目经理看板剩余2个模块补全创建能力，至此全部6个模块均支持查看+创建：

1. **支出记录** `FinanceModal` — 新增创建表单
   - 类型选择（支出/收入/备用金）
   - 项目选择下拉框
   - 用途说明、金额、分类、日期
   - 调用已有 `POST /finance` 端点

2. **供应商** `SupplierModal` — 新增创建表单
   - 供应商名称、联系人、电话、分类
   - 状态选择（潜在/合作中/已暂停）
   - 备注
   - 调用已有 `POST /suppliers` 端点

### 修改文件
- `packages/renderer/src/components/admin/role-dashboard/ProjectManagerDashboard.tsx`
  - FinanceModal: 重构增加创建表单（~120行新增）
  - SupplierModal: 重构增加创建表单（~95行新增）
...
- ✅ 生产部署（`/root/lingjing/server`）→ PM2重启 → OpenAPI 验证（GET+POST 均可见）

---

## 2026-06-04 v1.64.18 — 耗材管理新增创建表单（CRUD完成）✅

### 完成内容
为项目经理看板的「耗材管理」模块添加新增耗材能力，至此所有6个模块均支持查看+创建：

1. **后端** `POST /materials` 端点
   - 写入 `biz_finance`，type='expense'，category='material'
   - 必填：material_desc（耗材描述）、amount（金额）
   - 可选：project_id、supplier_name、expense_date、remark
   - 状态自动设为 "approved"

2. **前端** `MaterialsModal` 新增创建表单
   - "+ 新增耗材"按钮切换列表/表单视图
   - 项目选择下拉框（从 GET /projects 加载）
   - 耗材描述*、金额*、供应商、日期、备注
   - 提交 → POST /materials → 自动刷新列表

### 修改文件
- `server/app/routers/tenant_admin.py` — 新增 POST /materials 端点（~30行）
- `packages/renderer/src/components/admin/role-dashboard/ProjectManagerDashboard.tsx` — MaterialsModal 重构增加创建表单（~140行新增）
...
- ✅ Python 语法检查通过
- ✅ TypeScript 类型检查通过（无新增错误）
- ✅ 模式与 QualityModal 创建表单一致（已验证可行）

---

## 2026-06-04 v1.64.17 — 项目经理看板剩余3个模块：质量看板、耗材管理、考勤查看 ✅

### 完成内容
项目经理看板的空白模块全部补全，3个新模态框可真实加载数据：

1. **质量看板** (`QualityModal`)
   - 调取 `/api/v1/tenant-admin/quality` 获取质检记录
   - 显示检测类型、结果（合格/不合格/待检）、检测人员、日期、问题描述
2. **耗材管理** (`MaterialsModal`)
   - 调取 `/api/v1/tenant-admin/materials` 获取耗材记录
   - 顶部汇总耗材总支出，列表展示明细
3. **考勤查看** (`AttendanceViewModal`)
   - 调取 `/api/v1/tenant-admin/attendance/summary` 获取今日考勤汇总
   - 三格卡片：已上班 / 已下班 / 在岗中

### 修改文件
- `packages/renderer/src/components/admin/role-dashboard/ProjectManagerDashboard.tsx`
  - 新增 `QualityModal` / `MaterialsModal` / `AttendanceViewModal` 三个组件
  - 新增 `showQuality` / `showMaterials` / `showAttendanceView` 三个状态
  - `handleModuleClick` 路由映射
  - 模板中渲染3个新模态框
  - `AttendanceViewModal` 同时展示汇总卡片（今日）和明细列表（最近打卡记录）
- `server/app/routers/tenant_admin.py`
  - 新增 `GET /attendance-all` 端点，返回工人考勤明细列表（含用户名）

### 风险 & 验证
- ✅ 前端 TypeScript 类型检查通过（无新增错误）
- ✅ 后端 Python 语法检查通过
- ✅ 3个后端 API 端点 (`/quality`, `/materials`, `/attendance-all`) 均已存在
- ✅ 考勤查看同时调用 `/attendance/summary`（汇总）+ `/attendance-all`（明细）

### 后续增强
- ✅ `/quality` 端点增加 `bp.name as project_name` LEFT JOIN，前端同时展示项目名称
- ✅ `AttendanceViewModal` 修复：summary 为空时不阻塞明细列表展示
- ✅ 生产环境部署：代码推送 → 生产拉取 → PM2 重启 → OpenAPI 验证通过
...
### V13 — 项目经理 + 租户管理员看板真功能模块
- **ProjectManagerDashboard**: 新增"支出记录" FinanceModal + "供应商" SupplierModal
- **TenantAdminDashboard**: 新增"资金管理" FinanceModal + "团队管理" TeamModal + "发票管理" InvoiceModal + "合同管理" ContractModal
- 所有模块调用对应后端 API，显示真实业务数据
- **至此三个角色看板全部模块均可使用** 🎉

### V12 — v1.64.16 版本号更新
- 4 个 `package.json` 同步更新: 1.64.15 → **1.64.16**
- 零功能、零UI、零逻辑变动
- 已推送 `prod/main`

### V11 功能 — 工人"我的项目" + "报工记录" 模块
- `ProjectModal`：展示工人绑定的项目详情（名称/状态/进度/项目经理/合同金额/地点/角色/工种/日薪）
- `WorkReportModal`：本月报工记录列表（按日展示上下班打卡时间）
- 后端新增 `GET /api/attendance/my-project/{user_id}` 端点
- **工人看板 6/6 模块全部实现真功能** 🎉

### V10 功能 — 工人"申请备用金"模块
- 新增 `FundApplyModal` 备用金申请模态框
- 输入金额 + 用途说明 → 提交到 `POST /api/attendance/fund-apply`
- 创建 biz_finance type='fund_application' 记录（status=pending）
- 提交后显示结果反馈（成功/失败）
- 后端新增 `POST /api/attendance/fund-apply` 端点
- 工人看板已有 **4/6 个模块实现真功能** ✅
