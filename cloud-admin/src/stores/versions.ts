...
export const useVersionStore = defineStore('versions', () => {
  ...
  async function loadList(): Promise<void> { loading.value = true; try { const res = await versionApi.list(); if (res.code === 0) list.value = res.data; } catch (e) { console.error("加载versions失败:", e); } finally { loading.value = false; } }
