/**
 * Deploy v1.50.0 Windows + Linux + Android to production server
 * v1.50.0: Quest Agent lifecycle fix(4HIGH) + Task halt fix(5) + Persistent vector memory + Mobile improvements
 * Usage: node scripts/deploy-v1500.mjs
 */
...
// Files are in release-v1480 (electron-builder cached old output dir)
const RELEASE_DIR = resolve(ROOT, `packages/electron/release-v1500`);
const FILES = [
...
`;
  ...
  // Step 5: Upload Android APK
  log('Step 5/6: Upload Android APK...');
  const apkLocal = resolve(ROOT, `lingjing-mobile-v${VERSION}.apk`);
  const apkRemote = `${REMOTE_DL}/LingJing-${VERSION}-android.apk`;
  if (existsSync(apkLocal)) {
    const sizeMB = (statSync(apkLocal).size / 1024 / 1024).toFixed(0);
    log(`  Upload LingJing-${VERSION}-android.apk (${sizeMB} MB)...`);
    const start = Date.now();
    await uploadFile(conn, apkLocal, apkRemote);
    const secs = ((Date.now() - start) / 1000).toFixed(1);
    log(`  Done (${secs}s)`);
  } else {
    log(`  SKIP APK (not found at ${apkLocal})`);
  }
  console.log('');

  // Step 6: Update versions.json
  log('Step 6/6: Update versions.json...');
  ...
  // Android
  if (existsSync(apkLocal)) {
    newEntry.files['android'] = { url: `LingJing-${VERSION}-android.apk`, size: statSync(apkLocal).size };
  }
  ...
  await execCmd(conn, `cat > ${REMOTE_DL}/versions.json << 'ENDJSON'\n${updatedJson}\nENDJSON`);
  log(`  versions.json updated (latest: ${VERSION})`);
  console.log('');

  // Restart update-server
  log('Restarting update-server...');
  await execCmd(conn, 'pm2 restart update-server 2>/dev/null || echo "pm2 not found"');
  log('Done');
