/**
 * Deploy v1.50.0 to production server (CommonJS version)
 */
...
async function main() {
  ...
  log('  Starting Linux build (this may take 5-10 minutes)...');
  const buildResult = await execCmd(conn, buildScript);
  log(`  Linux build done (exit code: ${buildResult.code})`);
  const outTail = buildResult.stdout.substring(Math.max(0, buildResult.stdout.length - 300));
  log(`  stdout tail: ${outTail}`);
  ...
  log(`  Found ${linuxFiles.length} Linux files`);
  ...
  // Copy Android APK if exists on server
  const apkFind = await execCmd(conn, `ls ${REMOTE_DL}/LingJing-*-android.apk 2>/dev/null | tail -1`);
  const existingApk = apkFind.stdout.trim();
  if (existingApk) {
    const apkName = `LingJing-${VERSION}-android.apk`;
    await execCmd(conn, `cp "${existingApk}" ${REMOTE_DL}/${apkName}`);
