...
export async function initDatabase(): Promise<SqlJsDatabase> {
  ...
  // App-wide key-value settings (encryption keys, preferences, etc.)
  db.run(`
  ...
  // Agent Session persistence (unified Chat + Quest session checkpointing)
  db.run(`
  ...
  // All migrations are version-tracked via schema_version table.
  // If any migration fails, the error propagates -> database is backed up & recreated.
  const migrations: { version: number; name: string; fn: () => Promise<void> }[] = [
    { version: 1, name: 'checkpoint_tables', fn: async () => { const { migration001 } = await import('./migrations/migration001_checkpoint_tables'); db!.run(migration001); } },
    { version: 2, name: 'platform_tables', fn: async () => { const { migration002 } = await import('./migrations/migration002_platform_tables'); db!.run(migration002); } },
    { version: 3, name: 'hermes_fusion', fn: async () => { const { migration003 } = await import('./migrations/migration003_hermes_fusion'); db!.run(migration003); } },
    { version: 4, name: 'openspace', fn: async () => { const { migration004 } = await import('./migrations/migration004_openspace'); db!.run(migration004); } },
    { version: 5, name: 'github_skills', fn: async () => { const { migration005 } = await import('./migrations/migration005_github_skills'); db!.run(migration005); } },
    { version: 6, name: 'session_persistence', fn: async () => { const { applyMigration006 } = await import('./migrations/migration006_session_persistence'); applyMigration006(db!); } },
    { version: 7, name: 'hw_skill', fn: async () => { const { applyMigration007 } = await import('./migrations/migration007_hw_skill'); applyMigration007(db!); } },
    { version: 8, name: 'skill_reviews', fn: async () => { const { migration008 } = await import('./migrations/migration008_skill_reviews'); db!.run(migration008); } },
  ];
...
export async function applyMigrations(
  db: SqlJsDatabase,
  migrations: { version: number; name: string; fn: () => Promise<void> }[],
): Promise<void> {
  // Get current version
  const res = db.exec('SELECT MAX(version) FROM schema_version');
  const currentVersion =
    res.length > 0 && res[0].values[0][0] !== null ? (res[0].values[0][0] as number) : 0;
  console.log('[DB] Current schema version: ' + currentVersion);

  for (const migration of migrations) {
    if (migration.version > currentVersion) {
      console.log('[DB] Applying migration ' + migration.name + ' (v' + migration.version + ')...');
      try {
        await Promise.race([
          migration.fn(),
          new Promise<void>((_, reject) =>
            setTimeout(() => reject(new Error('Migration ' + migration.name + ' timed out after 30s')), 30000)
          ),
        ]);
        ...
        console.error('[DB] Migration ' + migration.name + ' failed — skipping (degraded mode):', err);
        console.warn('[DB] Application will start with missing columns/tables from ' + migration.name);
        try {
          db.run('INSERT INTO schema_version (version) VALUES (' + migration.version + ')');
        } catch { /* ignore */ }
      }
...
export async function saveDatabase(): Promise<void> {
  ...
  saveLock = saveLock.then(async () => {
    ...
    // Recovery: if one save fails, reset the lock so future saves
    // are not permanently blocked by a rejected promise chain
    console.error('[DB] Save lock broken, resetting. Error:', err);
