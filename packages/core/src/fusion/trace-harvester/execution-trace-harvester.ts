...
import type { TraceHarvesterConfig, ToolCallStep, ExecutionTrace, WorkflowPattern } from './types.js';
...
export class ExecutionTraceHarvester {
  private config: TraceHarvesterConfig;
  private eventBus: IEventBus | null = null;
  private llmAdapter: ILLMAdapter | null = null;
  private traceBuffer = new Map<string, ToolCallStep[]>();
  ...
  initialize(eventBus: IEventBus, llmAdapter?: ILLMAdapter): void {
    ...
    this.eventBus.subscribe('agent:tool_call', (event) => {
      const data = event.data as { sessionId?: string; toolName?: string; parameters?: Record<string, unknown> };
    ...
    this.eventBus.subscribe('agent:tool_result', (event) => {
      const data = event.data as { sessionId?: string; result?: string; duration?: number };
      if (data.sessionId) {
  ...
  async analyzeAndGenerateSkill(sessionId: string): Promise<string | null> {
    if (!this.enabled) return null;

    const trace = this.collectTrace(sessionId);
    if (!trace || trace.toolCallSequence.length < this.config.minToolCalls) return null;

    const duration = trace.endTime - trace.startTime;
    if (duration < this.config.minTraceDuration) return null;
  ...
  private onToolCall(sessionId: string, toolName: string, parameters: Record<string, unknown>): void {
  ...
  private onToolResult(sessionId: string, result: string, duration: number): void {
  ...
  private async onMessageEnd(sessionId: string): Promise<void> {
  ...
  private async generateSkillMd(trace: ExecutionTrace, patterns: WorkflowPattern[]): Promise<string> {
    if (!this.llmAdapter) throw new Error('LLM adapter not available');
