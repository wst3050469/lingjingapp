...
export class McpClient extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    logger.info(
      `[MCP:${this.serverName}] Connected to ${result.serverInfo.name} v${result.serverInfo.version}, ${this._tools.length} tools available`,
    );
  }
  ...
  private request<T>(method: string, params: Record<string, unknown>, timeoutMs: number = 60000): Promise<T> {
