...
### 根因
灵境AI站点nginx配置中，`/downloads/` 路径块（用于 electron-updater feedURL）**缺失**。请求 `/downloads/latest.yml` 返回 301（重定向到非 `/downloads/` 路径），而不是 `200 OK`。
...
```
client → feedURL: https://灵境API/downloads/
...
| 生产API `/api/latest` | ✅ 200 |
| 生产API `/api/v1/app/check-update` | ✅ 200 |
