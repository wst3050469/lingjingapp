...
type AdminTab = 'dashboard' | 'audit' | 'config' | 'data' | 'versions' | 'webhooks';
  ...
  return (
    <div className="flex flex-col h-full" style={{
      ...
      <div className="flex-1 overflow-auto p-3">
        ...
        {activeTab === 'webhooks' && <WebhookTab cloudAdminApi={cloudAdminApi} />}
      </div>
...
function VersionTab({
  ...
  const platformLabels: Record<string, string> = {
    'win-x64': '🖥️ Windows 安装包',
    'win-x64-portable': '💼 Windows 便携版',
        ...
        <div className="divide-y divide-gray-700">
          {versions.length === 0 ? (
            ...
            versions.map(v => {
              ...
              const hasFiles = v.files && Object.keys(v.files).length > 0;
              return (
                <div key={v.id}>
                  <div className="px-4 py-3 flex items-center gap-3">
                    <div
                      className={`flex-1 min-w-0 ${hasFiles ? 'cursor-pointer' : ''}`}
                      onClick={() => hasFiles && toggleExpand(v.id)}
                    >
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {hasFiles && (
                          <span className="text-[9px] text-gray-600">
                            {isExpanded ? '▼' : '▶'} {Object.keys(v.files!).length} 平台
                          </span>
                        )}
                      </div>
                      {v.changelog && (
                  ...
                  {/* Expanded platform files */}
                  {isExpanded && hasFiles && (
                    <div className="px-4 pb-3 pl-8 border-t border-gray-700/50">
                      <div className="grid grid-cols-1 gap-1 pt-2">
                        {Object.entries(v.files!).map(([platform, url]) => (
                          <div key={platform} className="flex items-center gap-2 text-[10px]">
                            ...
                            <button
                              ...
                              className="text-[9px] px-1.5 py-0.5 rounded text-gray-500 hover:text-gray-300 hover:bg-gray-700/50 shrink-0"
                              title="复制链接"
                            >
                              📋
                            </button>
