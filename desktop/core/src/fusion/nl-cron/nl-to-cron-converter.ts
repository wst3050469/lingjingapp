...
const NL_CRON_MAP: Array<{ pattern: RegExp; cron: string }> = [
...
export class NLToCronConverter {
  convert(naturalLanguage: string, llmProvider?: LLMProvider): Promise<{ cron: string; error?: string }> {
    if (llmProvider) {
  ...
  private async tryLLMConversion(naturalLanguage: string, llmProvider: LLMProvider): Promise<{ cron: string; error?: string }> {
    try {
  ...
  private ruleBasedConvert(naturalLanguage: string): { cron: string; error?: string } {
  ...
  validateCron(cron: string): boolean {
    ...
    if (!this.validateField(dayOfMonth, 1, 31)) return false;
    if (!this.validateField(month, 1, 12)) return false;
    if (!this.validateField(dayOfWeek, 0, 6)) return false;
    return true;
  }
